#!/usr/bin/env python3
"""Materialize a cache-local spec13a intent-prompt workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec13a_v7.py"
...
def _build_stage_rows(
    ...
    *,
    ...
    for row in catalog_rows:
        ...
        repeat = int(anchor_repeat) if prompt_family == "explicit_anchor" else int(intent_repeat)
        repeat = max(1, repeat)
...
def _summarize_catalog(catalog_rows: list[dict[str, Any]], *, split: str) -> dict[str, Any]:
    ...
    for row in catalog_rows:
        if not isinstance(row, dict) or str(row.get("split") or "") != split or not bool(row.get("training_prompt")):
            continue
        prompt_family_counts[str(row.get("prompt_family") or "unknown")] += 1
...
def materialize_workspace(
    ...
    *,
    ...
    pretrain_anchor_repeat: int,
    pretrain_intent_repeat: int,
    midtrain_anchor_repeat: int,
    midtrain_intent_repeat: int,
    python_exec: str,
    force: bool,
) -> dict[str, Any]:
    ...
    pretrain_train_rows = _build_stage_rows(
        render_catalog_rows,
        split="train",
        anchor_repeat=int(pretrain_anchor_repeat),
        intent_repeat=int(pretrain_intent_repeat),
        base=base,
    )
    midtrain_train_rows = _build_stage_rows(
        render_catalog_rows,
        split="train",
        anchor_repeat=int(midtrain_anchor_repeat),
        intent_repeat=int(midtrain_intent_repeat),
        base=base,
    )
    ...
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec13a_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec13a probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    tokenizer_manifest = {
        ...
        "frozen_from_run": str(freeze_tokenizer_run.expanduser().resolve()),
        "artifacts": {
    ...
    workspace_manifest = {
        ...
        "pretrain_anchor_repeat": int(pretrain_anchor_repeat),
        "pretrain_intent_repeat": int(pretrain_intent_repeat),
        "midtrain_anchor_repeat": int(midtrain_anchor_repeat),
        "midtrain_intent_repeat": int(midtrain_intent_repeat),
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec13a uses whole-scene anchor + intent rows only. No fragment repair rows are added.",
            },
        },
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec13a scene DSL workspace")
    ...
    ap.add_argument("--pretrain-anchor-repeat", type=int, default=4)
    ap.add_argument("--pretrain-intent-repeat", type=int, default=6)
    ap.add_argument("--midtrain-anchor-repeat", type=int, default=3)
    ap.add_argument("--midtrain-intent-repeat", type=int, default=8)
    ...
    summary = materialize_workspace(
        ...
        pretrain_anchor_repeat=int(args.pretrain_anchor_repeat),
        pretrain_intent_repeat=int(args.pretrain_intent_repeat),
        midtrain_anchor_repeat=int(args.midtrain_anchor_repeat),
        midtrain_intent_repeat=int(args.midtrain_intent_repeat),
