...
RUN="${1:-$MODEL_CACHE_ROOT/train/spec15b_scene_dsl_l3_d192_h384_ctx768_r1}"
DATASET_PREFIX="${DATASET_PREFIX:-spec15b_scene_dsl}"
...
ensure_oneapi_runtime_libs() {
  ...
  if [ -n "$found" ]; then
    ...
    printf '[spec15b-env] oneapi_runtime_lib=%s\n' "$found"
  fi
}
...
clear_stale_postrun_artifacts() {
  local run_dir="$1"
  rm -f \
    "$run_dir/spec15b_probe_contract.json" \
    "$run_dir/spec15b_probe_report.json" \
    "$run_dir/spec15b_probe_report.html" \
    "$run_dir/spec15b_tested_prompts_report.html" \
    "$run_dir/spec15b_tested_prompts_report.md" \
...
run_timed() {
  ...
  printf '[spec15b-timing] stage=%s event=start started_at=%s\n' "$label" "$(timestamp_utc)"
  "$@"
  rc=$?
  ended_epoch="$(date +%s)"
  printf '[spec15b-timing] stage=%s event=end ended_at=%s elapsed_sec=%s\n' \
...
build_probe_artifacts() {
  ...
  python3 version/v7/scripts/build_spec15b_probe_contract_v7.py \
    --run "$run_dir" \
  ...
  python3 version/v7/scripts/build_probe_report_v7.py \
    ...
    --output "$run_dir/spec15b_probe_report.html" \
    --json-out "$run_dir/spec15b_probe_report.json"

  python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
    --probe-report "$run_dir/spec15b_probe_report.json" \
    --output-html "$run_dir/spec15b_tested_prompts_report.html" \
    --output-md "$run_dir/spec15b_tested_prompts_report.md"
...
RUN_SCOPE_FAMILY="${RUN_SCOPE_FAMILY:-system_diagram}"
...
PROBE_CONTRACT="$RUN/spec15b_probe_contract.json"
PREFLIGHT_JSON="$RUN/spec15b_preflight.json"
...
MATERIALIZE_ARGS=(
  python3
  version/v7/scripts/dataset/materialize_spec15b_scene_dsl_v7.py
  --workspace "$WORKSPACE"
  --prefix "$DATASET_PREFIX"
...
if [ "$RUN_PREFLIGHT" = "1" ]; then
  PREFLIGHT_ARGS=(
    python3
    version/v7/scripts/spec15b_preflight_v7.py
    --run "$RUN"
    --prefix "$DATASET_PREFIX"
...
printf '[spec15b-budget] pretrain_total_tokens=%s midtrain_total_tokens=%s\n' "$PRETRAIN_TOTAL_TOKENS" "$MIDTRAIN_TOTAL_TOKENS"
printf '[spec15b-telemetry] train_driver=%s ck_cli_log_every=%s analysis_checkpoints=%s train_save_every=%s train_save_final=%s\n' \
  "$TRAIN_DRIVER" "$CK_CLI_LOG_EVERY" "$ANALYSIS_CHECKPOINTS" "$TRAIN_SAVE_EVERY" "$TRAIN_SAVE_FINAL"
printf '[spec15b-policy] dataset_prefix=%s family=system_diagram\n' "$DATASET_PREFIX"
printf '[spec15b-policy] oracle=pytorch parity_regimen=python ck_runtime_bridge=ck_run production_train=ck_cli\n'
printf '[spec15b-policy] stage_a_mix=canonical:%s bridge:%s stage_b_mix=canonical:%s bridge:%s\n' \
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec15b_pretrain_midtrain_v7.sh",
...
PRETRAIN_CMD=(
  ...
  --lr 3e-4
  --no-post-train-eval
  --json-out "$RUN/train_spec15b_stage_a.json"
)
...
MIDTRAIN_CMD=(
  ...
  --lr 2.5e-4
  --no-post-train-eval
  --json-out "$RUN/train_spec15b_stage_b.json"
)
if [ "${#TRAIN_SAVE_FINAL_FLAG[@]}" -gt 0 ]; then
...
if [ "$RUN_STAGE_EVAL" = "1" ]; then
  ...
  echo "[stage-eval] skipped builtin default probes; use spec15b_probe_report.json/html for system_diagram evaluation."
fi

build_probe_artifacts "$RUN" "$PROBE_CONTRACT" "$DATASET_PREFIX" "$CONTEXT_LEN" "$PROBE_PER_SPLIT" "$HIDDEN_PROBE_PER_SPLIT"
printf 'spec15b training finished for %s\n' "$RUN"
