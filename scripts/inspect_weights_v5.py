#!/usr/bin/env python3
"""
inspect_weights_v5.py
=====================

Lightweight weights inspection for v5:
...
import convert_gguf_to_bump_v4 as gguf_v4
...
def dtype_name(dt: int) -> str:
    if dt == gguf_v4.CK_DT_Q4_K:
        return "q4_k"
    if dt == gguf_v4.CK_DT_Q6_K:
        return "q6_k"
    return "fp32"
...
def inspect_gguf(gguf_path: Path, max_layers: Optional[int]) -> Tuple[Dict, List[Dict]]:
    with open(gguf_path, "rb") as f:
        ...
        tok_dt = gguf_v4.weight_dtype(tok, "token_emb")
        manifest_entries.append({
            "name": "token_emb",
        ...
        for layer in range(num_layers):
            ...
            gate_dt = gguf_v4.weight_dtype(gate, "ffn_gate")
            up_dt = gguf_v4.weight_dtype(up, "ffn_up")
            if gate_dt != up_dt:
            ...
            manifest_entries.extend([
                ...
                {"name": f"layer.{layer}.wq", "dtype": dtype_name(gguf_v4.weight_dtype(wq, "attn_q")),
                 "shape": [wq.ne1, wq.ne0], "source_dtype": gguf.ggml_type_name(wq.ggml_type)},
                {"name": f"layer.{layer}.wk", "dtype": dtype_name(gguf_v4.weight_dtype(wk, "attn_k")),
                 "shape": [wk.ne1, wk.ne0], "source_dtype": gguf.ggml_type_name(wk.ggml_type)},
                {"name": f"layer.{layer}.wv", "dtype": dtype_name(gguf_v4.weight_dtype(wv, "attn_v")),
                 "shape": [wv.ne1, wv.ne0], "source_dtype": gguf.ggml_type_name(wv.ggml_type)},
                {"name": f"layer.{layer}.wo", "dtype": dtype_name(gguf_v4.weight_dtype(wo, "attn_output")),
                ...
                {"name": f"layer.{layer}.w2", "dtype": dtype_name(gguf_v4.weight_dtype(down, "ffn_down")),
        ...
        if out_weight is not None:
            out_dt = gguf_v4.weight_dtype(out_weight, "lm_head")
            manifest_entries.append({
                "name": "lm_head_weight",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Inspect weights and emit v5 manifest (no conversion).")
