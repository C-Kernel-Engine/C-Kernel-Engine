...
One-shot operator script that prepares all viewer artifacts for a v8 run:
...
Usage:
    python3 version/v8/tools/prepare_run_viewer_v8.py <run_dir>
    python3 version/v8/tools/prepare_run_viewer_v8.py <run_dir> --dry-run
...
DEFAULT_MODELS_ROOT = Path.home() / ".cache" / "ck-engine-v8" / "models"
...
def prepare_run(run_dir: Path, *, force: bool = False, dry_run: bool = False) -> dict:
    ...
    if has_wt and (force or not emb_path.exists()):
        ok = run_step(
            ...
            [sys.executable, str(SCRIPT_DIR / "export_embeddings_v8.py"), str(run_dir)],
            dry_run,
        )
    ...
    if has_wt and has_tok and (force or not attn_path.exists()):
        cmd = [sys.executable, str(SCRIPT_DIR / "export_attention_v8.py"), str(run_dir)]
        if has_probe:
            cmd.append("--probe")
        else:
            # No probe_report — try generating one from tokenizer vocab
            gen_script = REPO_ROOT / "version" / "v8" / "tools" / "generate_fallback_probe_v8.py"
            if gen_script.exists():
                ...
                print(f"  ⏭ skipping attention: no probe_report.json (run generate_fallback_probe_v8.py first)")
                status["attention"] = "skip"
                cmd = None
        if cmd is not None:
            ok = run_step(
    ...
    if ws and (force or not viewer_path.exists()):
        ok = run_step(
            ...
            [
                sys.executable,
                str(REPO_ROOT / "version" / "v8" / "scripts" / "build_svg_dataset_visualizer_v8.py"),
                "--workspace", str(ws),
        ...
        if snap.exists():
            try:
                ...
                if src_ws:
                    ...
                    print(f"    hint: try: python3 version/v8/scripts/materialize_svg_stage_artifacts_v8.py --workspace {src_ws} --force")
            except Exception:
                pass
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Prepare all viewer artifacts (embeddings, attention, dataset viewer) for v8 runs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Prepare one run:
  python3 version/v8/tools/prepare_run_viewer_v8.py ~/.cache/ck-engine-v8/models/train/toy_svg_atoms_ctx512_d64_h128

  # Dry-run to see what would happen:
  python3 version/v8/tools/prepare_run_viewer_v8.py ~/.cache/ck-engine-v8/models/train/toy_svg_atoms_ctx512_d64_h128 --dry-run

  # Force-regenerate everything:
  python3 version/v8/tools/prepare_run_viewer_v8.py ~/.cache/ck-engine-v8/models/train/toy_svg_atoms_ctx512_d64_h128 --force

  # Prepare ALL train runs:
  python3 version/v8/tools/prepare_run_viewer_v8.py --all

  # Regenerate IR hub after preparing:
  python3 version/v8/tools/open_ir_hub_v8.py --open
""",
    )
    ap.add_argument("run_dir", nargs="?", help="Path to a v8 run directory")
    ap.add_argument("--all", action="store_true", help="Process all train runs under ~/.cache/ck-engine-v8/models/")
    ...
    ap.add_argument("--models-root", type=Path, default=DEFAULT_MODELS_ROOT, help="Models root (default: ~/.cache/ck-engine-v8/models)")
    args = ap.parse_args()
    ...
    if any_changes and not args.dry_run:
        ...
        print(f"    python3 version/v8/tools/open_ir_hub_v8.py --open")
