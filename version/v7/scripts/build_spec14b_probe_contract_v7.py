#!/usr/bin/env python3
"""Build a balanced probe contract for spec14b timeline runs."""
...
def main() -> int:
    ...
    ap.add_argument("--hidden-per-split", type=int, default=4)
    args = ap.parse_args()

    prefix = _detect_prefix_local(args.run.expanduser().resolve(), args.prefix)
    contract = build_contract(args.run, prefix, int(args.per_split), hidden_per_split=int(args.hidden_per_split))
