#!/usr/bin/env python3
"""Parser + canonicalizer for spec12 scene DSL."""
...
LAYOUTS = {"table_matrix", "decision_tree", "memory_map", "flow_graph"}
...
def parse_scene_document(text: str) -> SceneDocument:
    ...
    if not tokens or tokens[0] != "[scene]":
        raise ValueError("spec12 scene DSL must start with [scene]")
    if "[/scene]" not in tokens:
        raise ValueError("spec12 scene DSL must end with [/scene]")
    ...
    for token in tokens[1:-1]:
        ...
        if token.startswith("[") and token.endswith("]") and ":" in token:
            ...
            continue
        raise ValueError(f"unsupported token in spec12 scene: {token}")
...
def canonicalize_scene(scene: SceneDocument) -> SceneDocument:
    if scene.layout not in LAYOUTS:
        raise ValueError(f"unsupported spec12 layout: {scene.layout or '<empty>'}")
    if scene.theme not in THEMES:
        raise ValueError(f"unsupported spec12 theme: {scene.theme}")
    if scene.tone not in TONES:
        raise ValueError(f"unsupported spec12 tone: {scene.tone}")
    if scene.density not in DENSITIES:
        raise ValueError(f"unsupported spec12 density: {scene.density}")
    if scene.canvas not in CANVAS:
        raise ValueError(f"unsupported spec12 canvas: {scene.canvas}")
    if scene.frame not in FRAMES:
        raise ValueError(f"unsupported spec12 frame: {scene.frame}")
    if scene.background not in BACKGROUNDS:
        raise ValueError(f"unsupported spec12 background: {scene.background}")

    validated: list[SceneComponent] = []
    for component in scene.components:
        if component.name not in BLOCK_COMPONENTS:
            raise ValueError(f"unsupported spec12 component: {component.name}")
