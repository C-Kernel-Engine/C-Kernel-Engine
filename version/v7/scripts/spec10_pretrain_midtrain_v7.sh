...
RUN="${1:-$MODEL_CACHE_ROOT/train/spec10_asset_scene_dsl_l3_d192_h384_ctx512}"
DATASET_PREFIX="${DATASET_PREFIX:-spec10_asset_scene_dsl}"
...
PROBE_CONTRACT="$RUN/spec10_probe_contract.json"
PREFLIGHT_JSON="$RUN/spec10_preflight.json"

cd "$ROOT"
mkdir -p "$RUN"

python3 version/v7/scripts/dataset/materialize_spec10_scene_dsl_v7.py \
  --workspace "$WORKSPACE" \
...
if [ "$RUN_PREFLIGHT" = "1" ]; then
  PREFLIGHT_ARGS=(
    python3
    version/v7/scripts/spec10_preflight_v7.py
    --run "$RUN"
    --prefix "$DATASET_PREFIX"
...
printf '[spec10-budget] pretrain_total_tokens=%s midtrain_total_tokens=%s\n' "$PRETRAIN_TOTAL_TOKENS" "$MIDTRAIN_TOTAL_TOKENS"
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec10_pretrain_midtrain_v7.sh",
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 3e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec10_stage_a.json"
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 2.5e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec10_stage_b.json"
...
if [ "$RUN_STAGE_EVAL" = "1" ]; then
  ...
  echo "[stage-eval] skipped builtin default probes; use spec10_probe_report.json/html for asset-grounded scene evaluation."
fi
...
python3 version/v7/scripts/build_probe_report_v7.py \
  ...
  --output "$RUN/spec10_probe_report.html" \
  --json-out "$RUN/spec10_probe_report.json"

python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
  --probe-report "$RUN/spec10_probe_report.json" \
  --output-html "$RUN/spec10_tested_prompts_report.html" \
  --output-md "$RUN/spec10_tested_prompts_report.md"
...
printf 'spec10 pretrain+midtrain finished for %s\n' "$RUN"
