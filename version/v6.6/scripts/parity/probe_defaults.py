...
def getenv_prefill_tokens_csv() -> str:
    return os.environ.get("CK_V66_PROBE_PREFILL_TOKENS", DEFAULT_PREFILL_TOKENS_CSV)


def getenv_decode_token_id() -> int:
    raw = os.environ.get("CK_V66_PROBE_DECODE_TOKEN")
    if raw is None:
...
def getenv_autocheck_probe_token_id() -> int:
    raw = os.environ.get("CK_V66_AUTOCHECK_PROBE_TOKEN")
    if raw is None:
