/**
 * @file generic_api_test.c
 * @brief AUTO-GENERATED: model Implementation (IR v6.6 - Explicit Unrolled)
 *
 * Generated: 2026-01-12T04:22:25.186752 UTC
...
#error "model: v6.6 codegen currently supports fp32 only. Use --dtype=fp32."
#endif
...
/* ============================================================================
 * EXPLICIT PER-LAYER PREFILL FUNCTIONS (v6.6 unrolled)
 * ============================================================================ */
...
/* ============================================================================
 * EXPLICIT PER-LAYER DECODE FUNCTIONS (v6.6 unrolled)
 * ============================================================================ */
...
static void model_layer_0_decode(
    ...
    /* K projection: Q4_K -> gemm_nt_q4_k (direct-to-cache) */
    ...
    for (int h = 0; h < H_kv; ++h) {
        ...
        gemm_nt_q4_k(ln1_out, wk_h, bk_h, k_head, 1, head_dim, aligned_embed_dim);
        for (int d = head_dim; d < aligned_head_dim; ++d) {
            k_head[d] = 0.0f;
        }
    }

    /* V projection: Q4_K -> gemm_nt_q4_k (direct-to-cache) */
    ...
    for (int h = 0; h < H_kv; ++h) {
        ...
        gemm_nt_q4_k(ln1_out, wv_h, bv_h, v_head, 1, head_dim, aligned_embed_dim);
        for (int d = head_dim; d < aligned_head_dim; ++d) {
            v_head[d] = 0.0f;
        }
    }
...
static void model_decode_token(
    ...
    gemm_nt_q4_k(final_out, lm_head, NULL, logits, 1, MODEL_VOCAB_SIZE, aligned_embed_dim);
}
...
void *ck_model_create(void) {
    MODELModel *model = malloc(sizeof(MODELModel));
    if (!model) return NULL;
    if (model_model_allocate(model) != 0) {
        free(model);
        return NULL;
    }
    return model;
...
void ck_model_free(void *model) {
    if (!model) return;
    model_model_free((MODELModel *)model);
    free(model);
}

void ck_model_precompute_rope(void *model) {
    model_precompute_rope((MODELModel *)model);
}

void ck_model_forward(void *model, const int *tokens, int num_tokens) {
    model_forward((MODELModel *)model, tokens, num_tokens);
}

void ck_model_decode(void *model, const int *token, int token_index) {
    model_decode((MODELModel *)model, token, token_index);
}

float *ck_model_get_logits(void *model) {
    MODELModel *m = (MODELModel *)model;
    return MODEL_PTR(m, MODEL_FOOTER.logits);
}

int ck_model_verify_canaries(void *model) {
    return model_verify_canaries((MODELModel *)model);
}

void *ck_model_get_base(void *model) {
    return ((MODELModel *)model)->base;
}

size_t ck_model_get_total_bytes(void *model) {
    return ((MODELModel *)model)->total_bytes;
}
