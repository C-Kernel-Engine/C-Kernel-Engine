...
function buildDataPrepDiagnosisSection(files, pipeline) {
    ...
    const isToyAtomsRun = Boolean(
        files.train_toy_svg_atoms_stage_a
        || files.train_toy_svg_atoms_stage_a_continue
        || files.train_toy_svg_atoms_stage_a_overfit
        || (runDir && runDir.includes('toy_svg_atoms'))
    );
    const isToyStructuredRun = Boolean(
        files.train_structured_svg_atoms_stage_a
        || (runDir && runDir.includes('toy_svg_structured_atoms'))
    );
    const isToySemanticRun = Boolean(
        files.train_semantic_shapes_stage_a
        || (runDir && runDir.includes('toy_svg_semantic_shapes'))
    );
    ...
    const toyReportPath = runDir ? `${runDir}/toy_svg_semantic_probe_report.html` : '<run_dir>/toy_svg_semantic_probe_report.html';
    const toyReportCmd = runDir
        ? `python3 version/v7/scripts/build_toy_svg_semantic_report_v7.py --run-dir ${quoteShell(runDir)}`
        : 'python3 version/v7/scripts/build_toy_svg_semantic_report_v7.py --run-dir <run_dir>';
    const toyReportHref = runDir ? encodeURI(`file://${toyReportPath}`) : '';
    const toyAtomsReportPath = runDir ? `${runDir}/toy_svg_atoms_probe_report.html` : '<run_dir>/toy_svg_atoms_probe_report.html';
    const toyAtomsReportCmd = runDir
        ? `python3 version/v7/scripts/build_toy_svg_atoms_report_v7.py --run-dir ${quoteShell(runDir)}`
        : 'python3 version/v7/scripts/build_toy_svg_atoms_report_v7.py --run-dir <run_dir>';
    const toyAtomsReportHref = runDir ? encodeURI(`file://${toyAtomsReportPath}`) : '';
    const toyStructuredReportPath = runDir ? `${runDir}/toy_svg_structured_probe_report.html` : '<run_dir>/toy_svg_structured_probe_report.html';
    const toyStructuredReportCmd = runDir
        ? `python3 version/v7/scripts/build_toy_svg_structured_report_v7.py --run-dir ${quoteShell(runDir)}`
        : 'python3 version/v7/scripts/build_toy_svg_structured_report_v7.py --run-dir <run_dir>';
    const toyStructuredReportHref = runDir ? encodeURI(`file://${toyStructuredReportPath}`) : '';
    ...
    if (isToyAtomsRun) {
        diagnosisCards.push({
            title: 'TOY ATOMS REPORT',
            description: 'Runs seen and holdout prompts for the original raw-SVG toy run, extracts the first SVG, and compares it against expected targets.',
            cmd: toyAtomsReportCmd,
            href: toyAtomsReportHref,
            linkLabel: 'Open toy atoms report if generated',
        });
    }
    if (isToyStructuredRun) {
        diagnosisCards.push({
            title: 'TOY STRUCTURED REPORT',
            description: 'Runs seen and holdout probes for the structured SVG IR toy, renders the generated atoms, and compares them against expected targets.',
            cmd: toyStructuredReportCmd,
            href: toyStructuredReportHref,
            linkLabel: 'Open toy structured report if generated',
        });
    }
    if (isToySemanticRun) {
        diagnosisCards.push({
            title: 'TOY PROBE REPORT',
            description: 'Runs seen, holdout, and future-stage prompts, renders the first valid SVG IR, and compares it to expected targets.',
            cmd: toyReportCmd,
            href: toyReportHref,
            linkLabel: 'Open toy probe report if generated',
        });
    }
