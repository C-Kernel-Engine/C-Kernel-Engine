...
function buildDataPrepDiagnosisSection(files, pipeline) {
    ...
    if (!matrix || !Array.isArray(matrix.entries) || matrix.entries.length === 0) {
        diagnosisCards.push({
            title: 'STAGE EVAL',
            description: 'Generates the stage eval matrix used by the generic training surfaces and compare pages.',
            cmd: evalCmd,
            href: '',
            linkLabel: '',
        });
    } else if (datasetType === 'svg') {
        diagnosisCards.push({
            title: 'POST-RUN DEEP DIAGNOSIS',
            description: 'Generates a standalone report with stage metrics, rendered sample outputs, and dataset-vs-target gaps.',
            cmd: reportCardCmd,
            href: reportHref,
            linkLabel: 'Open report card if generated',
        });
    }
    const diagnosisCardsHtml = diagnosisCards.map((card) => `
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:0.7rem 0.8rem;">
            <div style="font-size:0.68rem;color:#9ca3af;font-weight:700;letter-spacing:0.07em;margin-bottom:0.25rem;">${htmlEscape(card.title)}</div>
            <div style="font-size:0.73rem;color:var(--text-muted);line-height:1.55;margin-bottom:0.4rem;">
                ${htmlEscape(card.description)}
            </div>
            <pre style="font-size:0.76rem;white-space:pre-wrap;margin:0;">${htmlEscape(card.cmd)}</pre>
            ${card.href ? `<div style="margin-top:0.35rem;font-size:0.72rem;"><a href="${htmlEscape(card.href)}" target="_blank" rel="noopener" style="color:#93c5fd;">${htmlEscape(card.linkLabel)}</a></div>` : ''}
        </div>
    `).join('') || `
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:0.7rem 0.8rem;">
            <div style="font-size:0.68rem;color:#9ca3af;font-weight:700;letter-spacing:0.07em;margin-bottom:0.25rem;">GENERIC DIAGNOSIS SURFACE</div>
            <div style="font-size:0.73rem;color:var(--text-muted);line-height:1.55;">
                No reusable post-run report adapter is registered for dataset type ${htmlEscape(datasetType)} yet. Keep the eval matrix and provenance audit generic until that adapter exists at the family level.
            </div>
        </div>
    `;
    ...
    const datasetTypeNote = datasetType === 'svg'
        ...
        : `Dataset type: ${htmlEscape(datasetType)}. This surface stays generic until a family-level diagnosis adapter is registered.`;
    ...
    const actionNote = datasetType === 'svg'
        ...
        : 'Operator use: audit provenance before training, promote from the eval matrix, and only add a report adapter when it is reusable across a model family rather than tied to one run.';
...
function buildDataLabParitySection(files) {
    ...
    const parityCmd = runCtx.runDir
        ? `python3 version/v7/scripts/ck_run_v7.py parity --run ${runDir} --backend ck --parity-on --train-epochs 1 --train-seq-len 8 --train-total-tokens 512 --train-grad-accum 8 --no-train-save-final`
        : 'python3 version/v7/scripts/ck_run_v7.py parity --run <run_dir> --backend ck --parity-on --train-epochs 1 --train-seq-len 8 --train-total-tokens 512 --train-grad-accum 8 --no-train-save-final';
    const regimenCmd = runCtx.runDir
        ? `python3 version/v7/scripts/run_training_parity_regimen_v7.py --run-dir ${runDir} --json-out ${quoteShell(`${runCtx.runDir}/training_parity_regimen_latest.json`)} --md-out ${quoteShell(`${runCtx.runDir}/training_parity_regimen_latest.md`)}`
        : 'python3 version/v7/scripts/run_training_parity_regimen_v7.py --run-dir <run_dir> --json-out <run_dir>/training_parity_regimen_latest.json --md-out <run_dir>/training_parity_regimen_latest.md';
    const rows = parseParityRows(files.training_parity);
    ...
    const regimen = files.training_parity_regimen && typeof files.training_parity_regimen === 'object'
        ...
        : {};
    const regimenStages = normalizeRegimenStages(regimen);
    const regimenOverall = regimenState(regimen, regimenStages);
    const regimenStatus = String(regimenOverall.label || '').toLowerCase();
    ...
    const regimenProgress = Number.isFinite(Number(regimenSummary.passed_stages)) && Number.isFinite(Number(regimenSummary.total_stages))
        ...
        : (regimenStages.length > 0
            ? `${regimenStages.filter((row) => String(row.status || '').toUpperCase() === 'PASS').length}/${regimenStages.length} stages`
    ...
    return `
        <div class="parity-section" style="margin-top:0.8rem;">
            ...
            ${regimenStages.length === 0 ? `
                <div style="margin-top:0.6rem;background:rgba(59,130,246,0.08);border:1px solid rgba(96,165,250,0.35);border-radius:6px;padding:0.55rem 0.75rem;font-size:0.73rem;color:#cbd5e1;line-height:1.6;">
                    A/B/C/D/E/F parity is missing for this run. <code>eval_stage_v7.py</code> does not create the parity regimen report. Run:
                    <pre style="font-size:0.74rem;white-space:pre-wrap;margin-top:0.45rem;">${htmlEscape(regimenCmd)}</pre>
                </div>
            ` : ''}
            ${openTrackerButton}
        </div>
...
// Operator-required guardrail:
// Keep the A/B/C/D/E/F readiness surface in Data Lab. If this disappears,
// downstream loss / SVG quality conclusions are no longer anchored to visible parity.
function buildDataLabReadinessSection(files) {
    const runCtx = getRunContext();
    const runDir = runCtx.runDir ? quoteShell(runCtx.runDir) : '<run_dir>';
    const regimenCmd = runCtx.runDir
        ? `python3 version/v7/scripts/run_training_parity_regimen_v7.py --run-dir ${runDir} --json-out ${quoteShell(`${runCtx.runDir}/training_parity_regimen_latest.json`)} --md-out ${quoteShell(`${runCtx.runDir}/training_parity_regimen_latest.md`)}`
    ...
    const regimen = files.training_parity_regimen && typeof files.training_parity_regimen === 'object'
        ...
        : {};
    const regimenStages = normalizeRegimenStages(regimen);
    const catalog = parityStageCatalog(regimenStages);
    const displayStages = regimenDisplayStages(regimenStages, catalog);
    const regimenOverall = regimenState(regimen, regimenStages);
    const abcGate = regimenGroupState(regimenStages, ['A', 'B', 'C']);
    const defGate = regimenGroupState(regimenStages, ['D', 'E', 'F']);
    const criticalFails = ['A1', 'D1', 'E1', 'F1'].filter((id) => {
        const row = regimenStages.find((stage) => String(stage.id || '').toUpperCase() === id);
        return row && String(row.status || '').toUpperCase() !== 'PASS';
    });

    const groupOrder = ['D/E/F - Generated C Runtime', 'A - Baseline Kernels', 'B - Grad Accum', 'C - Stability'];
    const byGroup = {};
    groupOrder.forEach((group) => { byGroup[group] = []; });
    displayStages.forEach((row) => {
        const id = String(row.id || '').trim().toUpperCase();
        if (!id) return;
        const info = catalog[id] || {
            group: 'Other',
            name: id,
            tests: String(row.name || ''),
            why: '',
        };
        if (!byGroup[info.group]) byGroup[info.group] = [];
        byGroup[info.group].push({ id, row, info });
    });

    const fmtDur = (seconds) => {
        const n = Number(seconds);
        if (!Number.isFinite(n)) return '';
        return n < 60 ? `${n.toFixed(1)}s` : `${(n / 60).toFixed(1)}m`;
    };
    const banner = (() => {
        if (regimenStages.length === 0) {
            return {
                color: '#93c5fd',
                text: 'Parity regimen not loaded - A/B/C/D/E/F cards are shown as required gates, but readiness is unknown until the regimen is generated.',
            };
        }
        if (regimenOverall.pass === true) {
            return {
                color: '#6ee7b7',
                text: 'All gate stages passing - production runtime verified, code is ready to train',
            };
        }
        if (criticalFails.length > 0) {
            return {
                color: '#fca5a5',
                text: `Critical gates failing (${criticalFails.join(', ')}) - do not start a long training run`,
            };
        }
        return {
            color: '#fbbf24',
            text: 'Some gates failing - review A/B/C/D/E/F results before committing training budget',
        };
    })();
    ...
    const groupHtml = groupOrder.map((group) => {
        ...
        const passCount = items.filter(({ row }) => String(row.status || '').toUpperCase() === 'PASS').length;
        const missingCount = items.filter(({ row }) => String(row.status || '').toUpperCase() === 'MISSING').length;
        const groupColor = missingCount === items.length
            ? '#93c5fd'
            : (passCount === items.length ? '#6ee7b7' : (passCount === 0 ? '#fca5a5' : '#fbbf24'));
        const cards = items.map(({ id, row, info }) => {
            const status = String(row.status || '').toUpperCase() || 'MISSING';
            const pass = status === 'PASS';
            const missing = status === 'MISSING';
            const border = missing ? '#60a5fa' : (pass ? '#47b475' : '#f87171');
            const bg = missing ? 'rgba(96,165,250,0.07)' : (pass ? 'rgba(71,180,117,0.07)' : 'rgba(248,113,113,0.07)');
            const tone = missing ? 'blue' : (pass ? 'green' : 'red');
            const metrics = row && typeof row.metrics === 'object' ? row.metrics : {};
            const metricBits = [];
            const lossDiff = Number(metrics.max_loss_abs_diff);
            const meanParam = Number(metrics.final_param_mean_abs_diff);
            const maxParam = Number(metrics.final_param_max_abs_diff);
            const optimizerSteps = Number(metrics.optimizer_steps);
            if (Number.isFinite(lossDiff)) metricBits.push(`lossΔ=${lossDiff.toExponential(2)}`);
            if (Number.isFinite(meanParam)) metricBits.push(`mean_paramΔ=${meanParam.toExponential(2)}`);
            else if (Number.isFinite(maxParam)) metricBits.push(`paramΔ=${maxParam.toExponential(2)}`);
            if (Number.isFinite(optimizerSteps)) metricBits.push(`${Math.round(optimizerSteps)} opt steps`);
            const notes = Array.isArray(row.notes) ? row.notes.filter(Boolean) : [];
            const note = notes.length > 0 ? String(notes[notes.length - 1]) : '';
            return `
                <div style="border:1px solid ${border};border-radius:6px;padding:0.5rem 0.65rem;background:${bg};box-shadow:inset 0 0 0 1px rgba(255,255,255,0.03);flex:1 1 215px;min-width:205px;max-width:360px;">
                    <div style="display:flex;align-items:center;gap:0.35rem;margin-bottom:0.16rem;">
                        ${statusPill(id, tone)}
                        ${statusPill(status, tone)}
                        <span style="font-size:0.8rem;font-weight:700;color:#e5e7eb;">${htmlEscape(info.name)}</span>
                        ${fmtDur(row.duration_s) ? `<span style="color:#6b7280;font-size:0.67rem;margin-left:auto;">${htmlEscape(fmtDur(row.duration_s))}</span>` : ''}
                    </div>
                    <div style="font-size:0.71rem;color:#9ca3af;line-height:1.42;">${htmlEscape(info.tests)}</div>
                    ${metricBits.length > 0 ? `<div style="font-size:0.69rem;color:#60a5fa;margin-top:0.22rem;font-family:monospace;">${htmlEscape(metricBits.join(' · '))}</div>` : ''}
                    ${note ? `<div style="font-size:0.67rem;color:#fbbf24;margin-top:0.14rem;">${htmlEscape(note)}</div>` : ''}
                    <div style="font-size:0.67rem;color:#a78bfa;margin-top:0.18rem;line-height:1.35;">↳ ${htmlEscape(info.why)}</div>
                </div>
            `;
        }).join('');
        const groupSummary = missingCount === items.length
            ? `${missingCount}/${items.length} missing`
            : `${passCount}/${items.length} pass${missingCount > 0 ? ` · ${missingCount} missing` : ''}`;
        return `
            <div style="margin-bottom:0.9rem;">
                <div style="font-size:0.72rem;font-weight:700;color:${groupColor};margin-bottom:0.38rem;letter-spacing:0.03em;text-transform:uppercase;">
                    ${htmlEscape(group)} <span style="opacity:0.6;font-weight:400;text-transform:none;">(${htmlEscape(groupSummary)})</span>
                </div>
    ...
    return `
        <div class="parity-section" style="margin-top:0.8rem;">
            <h3><span class="badge ${regimenOverall.pass === true ? 'badge-green' : 'badge-blue'}">Training Readiness</span> Parity Gate - Stage-by-Stage</h3>
            <div style="border-radius:6px;padding:0.55rem 0.9rem;margin-bottom:0.7rem;font-size:0.84rem;font-weight:700;color:${banner.color};border:1px solid ${banner.color};background:${banner.color}1a;">
                ${htmlEscape(banner.text)}
            </div>
            <div style="font-size:0.71rem;color:#6b7280;margin-bottom:0.7rem;line-height:1.6;">
                Each card shows <strong style="color:#9ca3af;">what the test verifies</strong> · <span style="color:#60a5fa;font-family:monospace;">key metrics</span> · <span style="color:#a78bfa;">why it matters</span>.
                <span style="color:#6ee7b7;font-weight:700;">D/E/F = production gate</span><span style="color:#9ca3af;"> — generated C runtime, all must pass before training.</span>
                <span style="color:#9ca3af;"> A/B/C = kernel isolation (diagnostic); useful for root-cause, but not a substitute for D/E/F.</span>
                <div style="margin-top:0.2rem;">
                    • regimen(A/B/C): <strong>${htmlEscape(abcGate.label)}</strong> <span style="color:#9ca3af;">${htmlEscape(abcGate.detail)}</span><br/>
                    • generated-runtime(D/E/F): <strong>${htmlEscape(defGate.label)}</strong> <span style="color:#9ca3af;">${htmlEscape(defGate.detail)}</span>
                </div>
            </div>
            ${regimenStages.length === 0 ? `
                <div style="background:rgba(59,130,246,0.08);border:1px solid rgba(96,165,250,0.35);border-radius:6px;padding:0.55rem 0.75rem;margin-bottom:0.75rem;font-size:0.73rem;color:#cbd5e1;line-height:1.6;">
                    A/B/C/D/E/F readiness does <strong>not</strong> come from <code>eval_stage_v7.py</code>. Generate <code>training_parity_regimen_latest.json</code> for this run, then regenerate <code>ir_report.html</code>.
                    <pre style="font-size:0.74rem;white-space:pre-wrap;margin-top:0.45rem;">${htmlEscape(regimenCmd)}</pre>
                </div>
            ` : ''}
            ${groupHtml}
...
function renderDataLabPanel(files) {
    ...
    const readinessSection = buildDataLabReadinessSection(files);
    ...
    root.innerHTML = `
        <div style="background:rgba(71,180,117,0.05);border:1px solid rgba(71,180,117,0.17);border-radius:8px;padding:0.55rem 0.85rem;margin-bottom:0.75rem;">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(175px,1fr));gap:0.28rem 0.65rem;">
                ...
                <div style="font-size:0.71rem;color:#9ca3af;"><span style="color:#f0abfc;font-weight:600;">Training Readiness</span> — A/B/C/D/E/F gates with D/E/F as the real generated-runtime signoff</div>
...
function normalizeRegimenStages(regimen) {
    const rows = Array.isArray(regimen && regimen.stages) ? regimen.stages
        : (Array.isArray(regimen && regimen.stage_results) ? regimen.stage_results : []);
    return rows
        .filter((row) => row && typeof row === 'object')
        .map((row) => ({
            id: String(row.id || row.stage_id || '').trim(),
            name: String(row.name || row.label || '').trim(),
            status: String(row.status || row.result || '').trim().toUpperCase(),
...
function parityStageCatalog(regimenStages = []) {
    const catalog = {
        A1: {
            group: 'A - Baseline Kernels',
            name: 'Fwd / Bwd / SGD',
            tests: 'Single micro-step forward+backward+SGD update. Lowest-bar numeric smoke test before spending training budget.',
            why: 'If A1 fails, basic forward/backward math is broken.',
        },
        A3: {
            group: 'A - Baseline Kernels',
            name: 'Backend Xray',
            tests: 'Swap rmsnorm / swiglu / loss backends one at a time on identical weights to isolate drift source.',
            why: 'This localizes divergence to a specific kernel family quickly.',
        },
        A4: {
            group: 'A - Baseline Kernels',
            name: 'AdamW Strict Sentinel',
            tests: 'Fixed strict AdamW sentinel on a tiny immutable model with locked tolerances.',
            why: 'This is the stable early warning for optimizer regressions.',
        },
        B2: {
            group: 'B - Grad Accum',
            name: 'AdamW g=2',
            tests: 'AdamW with grad_accum=2. Binding gate is mean parameter drift, not max, to avoid expected AdamW epsilon outliers.',
            why: 'Confirms gradient averaging is correct for 2x effective batch size.',
        },
        B4: {
            group: 'B - Grad Accum',
            name: 'AdamW g=4',
            tests: 'AdamW with grad_accum=4. Binding gate is mean parameter drift, not max, to avoid expected AdamW epsilon outliers.',
            why: 'Confirms gradient averaging is correct for 4x effective batch size.',
        },
        B8: {
            group: 'B - Grad Accum',
            name: 'AdamW g=8',
            tests: 'AdamW with grad_accum=8. Binding gate is mean parameter drift, not max, to avoid expected AdamW epsilon outliers.',
            why: 'Confirms gradient averaging is correct for 8x effective batch size.',
        },
        C1: {
            group: 'C - Stability',
            name: 'Stability Sweep C1',
            tests: 'Multi-epoch stability sweep looking for longer-horizon optimizer drift.',
            why: 'These catch issues that do not show up in the short A/B canaries.',
        },
        C2: {
            group: 'C - Stability',
            name: 'Stability Sweep C2',
            tests: 'Multi-epoch stability sweep looking for longer-horizon optimizer drift.',
            why: 'These catch issues that do not show up in the short A/B canaries.',
        },
        C3: {
            group: 'C - Stability',
            name: 'Stability Sweep C3',
            tests: 'Multi-epoch stability sweep looking for longer-horizon optimizer drift.',
            why: 'These catch issues that do not show up in the short A/B canaries.',
        },
        D1: {
            group: 'D/E/F - Generated C Runtime',
            name: 'Backprop Stitch',
            tests: 'Generated C forward+backward on a real sequence must match PyTorch gradients.',
            why: 'This is the first direct gate on the generated runtime that actually trains.',
        },
        D2: {
            group: 'D/E/F - Generated C Runtime',
            name: 'Stitch Multi-step',
            tests: 'Generated C multi-step train loop with grad_accum enabled.',
            why: 'This checks the real C training loop instead of only the Python harness.',
        },
        E1: {
            group: 'D/E/F - Generated C Runtime',
            name: 'Replay Determinism',
            tests: 'Re-running the same generated-C step should be bit-identical.',
            why: 'Non-determinism silently poisons gradient accumulation and replay.',
        },
        F1: {
            group: 'D/E/F - Generated C Runtime',
            name: 'Runtime Accum',
            tests: 'Accumulated generated-C gradients over micro-steps must match a single full-batch gradient.',
            why: 'If F1 fails, the runtime that actually trains is not trustworthy.',
        },
    };
    for (const row of regimenStages) {
        const id = String(row.id || '').trim().toUpperCase();
        if (/^B\d+$/.test(id) && !catalog[id]) {
            const g = id.slice(1);
            catalog[id] = {
                group: 'B - Grad Accum',
                name: `AdamW g=${g}`,
                tests: `AdamW with grad_accum=${g}. Binding gate is mean parameter drift, not max, to avoid expected AdamW epsilon outliers.`,
                why: `Confirms gradient averaging is correct for ${g}x effective batch size.`,
            };
        }
        if (/^C\d+$/.test(id) && !catalog[id]) {
            catalog[id] = {
                group: 'C - Stability',
...
function regimenDisplayStages(regimenStages, catalog) {
    const byId = new Map(Array.isArray(regimenStages)
        ? regimenStages.map((row) => [String(row.id || '').trim().toUpperCase(), row])
        : []);
    const rows = [];
    const seen = new Set();
    for (const id of Object.keys(catalog)) {
        ...
        rows.push(actual || {
            ...
            status: 'MISSING',
            metrics: {},
            notes: ['training_parity_regimen_latest.json missing for this run'],
            duration_s: null,
        });
        seen.add(id);
    }
    for (const row of Array.isArray(regimenStages) ? regimenStages : []) {
        const id = String(row.id || '').trim().toUpperCase();
...
function regimenState(regimen, stages) {
    const summary = regimen && typeof regimen.summary === 'object' ? regimen.summary : {};
    const status = String(regimen && regimen.status || '').trim().toLowerCase();
    const skipped = regimen && regimen.skipped === true;
    if (skipped) return { label: 'skipped', tone: 'blue', pass: null };
    if (status === 'pass' || summary.passed === true) return { label: 'pass', tone: 'green', pass: true };
    if (status === 'fail' || summary.passed === false) return { label: 'fail', tone: 'red', pass: false };
    if (Array.isArray(summary.failed_stage_ids) && summary.failed_stage_ids.length > 0) {
        return { label: 'fail', tone: 'red', pass: false };
    }
    if (Array.isArray(stages) && stages.length > 0) {
        const failing = stages.filter((row) => String(row.status || '').toUpperCase() !== 'PASS');
        return failing.length === 0
            ? { label: 'pass', tone: 'green', pass: true }
            : { label: 'fail', tone: 'red', pass: false };
    }
    return { label: 'missing', tone: 'orange', pass: null };
}

function regimenGroupState(stages, prefixes) {
    const want = Array.isArray(prefixes) ? prefixes.map((v) => String(v || '').toUpperCase()).filter(Boolean) : [];
    const matched = Array.isArray(stages)
        ? stages.filter((row) => {
            const id = String(row && row.id || '').toUpperCase();
            return want.some((prefix) => id.startsWith(prefix));
        })
        : [];
    if (matched.length === 0) {
        return { label: 'missing', tone: 'orange', pass: null, detail: 'no stages loaded' };
    }
    const failing = matched.filter((row) => String(row.status || '').toUpperCase() !== 'PASS');
    return {
        label: failing.length === 0 ? 'pass' : 'fail',
        tone: failing.length === 0 ? 'green' : 'red',
        pass: failing.length === 0,
        detail: matched.map((row) => `${row.id}:${row.status || '?'}`).join(' · '),
    };
}
...
function renderTrainingLogbookPanel(files) {
    ...
    const regimenOverall = regimenState(regimen, normalizeRegimenStages(regimen));
    if (regimenOverall.pass === false) {
        failures.push({
            title: 'Parity regimen failed',
            why: 'training_parity_regimen_latest.json indicates a failed gate',
            severity: 'error',
        });
