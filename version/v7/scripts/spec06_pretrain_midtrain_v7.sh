...
RUN="${1:-/tmp/spec06_structured_infographics_l3_d192_h384_ctx512}"
DATASET_PREFIX="${DATASET_PREFIX:-spec06_structured_infographics}"
...
PRETRAIN_TOTAL_TOKENS="${PRETRAIN_TOTAL_TOKENS:-1048576}"
MIDTRAIN_TOTAL_TOKENS="${MIDTRAIN_TOTAL_TOKENS:-1572864}"
...
PROBE_CONTRACT="$RUN/spec06_probe_contract.json"

cd "$ROOT"
mkdir -p "$RUN"

python3 version/v7/scripts/dataset/materialize_spec06_structured_atoms_v7.py \
  --workspace "$WORKSPACE" \
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec06_pretrain_midtrain_v7.sh",
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 3e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec06_stage_a.json"
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 2.5e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec06_stage_b.json"
...
if [ "$RUN_STAGE_EVAL" = "1" ]; then
  ...
  echo "[stage-eval] skipped builtin default probes; use spec06_probe_report.json/html for infographic evaluation."
fi

python3 version/v7/scripts/build_spec06_probe_contract_v7.py \
  --run "$RUN" \
  --prefix "$DATASET_PREFIX" \
...
python3 version/v7/scripts/build_probe_report_v7.py \
  ...
  --output "$RUN/spec06_probe_report.html" \
  --json-out "$RUN/spec06_probe_report.json"

python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
  --probe-report "$RUN/spec06_probe_report.json" \
  --output-html "$RUN/spec06_tested_prompts_report.html" \
  --output-md "$RUN/spec06_tested_prompts_report.md"
...
printf 'spec06 pretrain+midtrain finished for %s\n' "$RUN"
