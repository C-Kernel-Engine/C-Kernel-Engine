#!/usr/bin/env python3
"""Build a balanced probe contract for spec12 scene DSL runs."""
...
LAYOUT_ORDER = ("table_matrix", "decision_tree", "memory_map")
...
TOPIC_ORDER = ("failure_triage", "model_memory_layout", "quantization_formats")
...
def _selection_group_key(prompt: str, catalog: dict[str, dict[str, Any]]) -> tuple[str, ...]:
    ...
    case_id = str(row.get("case_id") or tags.get("topic", ""))
    return (tags.get("layout", ""), case_id, tags.get("theme", ""), tags.get("tone", ""))
...
def _pick_balanced_prompts(prompts: list[str], catalog: dict[str, dict[str, Any]], limit: int) -> list[str]:
    ...
    for layout in LAYOUT_ORDER:
        ...
        for prompt in layout_prompts:
            key = _selection_group_key(prompt, catalog)
...
def build_contract(run_dir: Path, prefix: str, per_split: int, *, hidden_per_split: int = 0) -> dict[str, Any]:
    ...
    if int(hidden_per_split) > 0:
        ...
        for split_name, split_label, path in hidden_candidates:
            if path.exists():
                split_defs.append((split_name, split_label, path))
    ...
    for split_name, split_label, path in split_defs:
        ...
        selected_prompts = _pick_balanced_prompts(prompts, catalog, max(1, limit))
        splits.append(
            {
                "name": split_name,
                "label": f"{split_label} prompts",
                "cases": [_case_from_prompt(prompt, catalog, split_name, split_label, index) for index, prompt in enumerate(selected_prompts, start=1)],
            }
        )

    return {
        "schema": "ck.probe_report_contract.v1",
        "title": f"{prefix} Scene DSL Probe Report",
        "dataset_type": "svg",
        "output_adapter": {
            "name": "text_renderer",
            "stop_markers": ["[/scene]"],
        ...
        "decode": {"max_tokens": decode_max_tokens, "temperature": 0.0},
        "splits": splits,
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Build a balanced probe contract for a spec12 scene DSL run")
    ap.add_argument("--run", required=True, type=Path)
    ap.add_argument("--prefix", default=None)
    ap.add_argument("--output", required=True, type=Path)
    ap.add_argument("--per-split", type=int, default=12)
    ap.add_argument("--hidden-per-split", type=int, default=0, help="Optional hidden paraphrase cases per hidden split when hidden prompt files exist")
    args = ap.parse_args()
    ...
    prefix = _detect_prefix_local(run_dir, str(args.prefix).strip() if args.prefix else None)
    if not prefix:
        prefix = "spec12_scene_dsl"

    contract = build_contract(run_dir, prefix, int(args.per_split), hidden_per_split=int(args.hidden_per_split))
