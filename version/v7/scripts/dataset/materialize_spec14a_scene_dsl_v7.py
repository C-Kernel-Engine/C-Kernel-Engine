#!/usr/bin/env python3
"""Materialize a cache-local spec14a comparison-board workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec14a_v7.py"
...
def _row_repeat(prompt_surface: str, *, canonical_repeat: int, bridge_repeat: int) -> int:
    ...
    prompt_surface_repeats = {
        "bridge_create": max(1, bridge_repeat // 3),
        "bridge_layout": max(1, bridge_repeat // 2),
        "bridge_stop": max(1, bridge_repeat),
        "bridge_anti_default": max(1, bridge_repeat + 2),
        "contrast_keep_default": max(1, bridge_repeat + 6),
        "contrast_from_default": max(1, bridge_repeat + 8),
        "repair_compiler_ready": max(1, bridge_repeat + 2),
        "repair_scene_only": max(1, bridge_repeat + 4),
        "repair_count_guard": max(1, bridge_repeat + 8),
        "repair_singleton_tags": max(1, bridge_repeat + 8),
        "repair_terminal_exact": max(1, bridge_repeat + 6),
        "repair_metric_guard": max(1, bridge_repeat + 8),
        "repair_callout_guard": max(1, bridge_repeat + 8),
        "repair_tag_exact": max(1, bridge_repeat + 12),
        "repair_hidden_compose_like": max(1, bridge_repeat + 10),
        "repair_hidden_compose_anchor": max(1, bridge_repeat + 8),
        "repair_hidden_stop_like": max(1, bridge_repeat + 12),
        "compose_control_blend": max(1, bridge_repeat + 10),
        "compose_scene_only": max(1, bridge_repeat + 8),
        "compose_anti_anchor": max(1, bridge_repeat + 10),
        "compose_hidden_compose_like": max(1, bridge_repeat + 10),
        "compose_hidden_stop_like": max(1, bridge_repeat + 12),
        "compose_terminal_exact": max(1, bridge_repeat + 6),
    }
    return prompt_surface_repeats.get(prompt_surface, max(1, bridge_repeat))


def _stage_group_key(row: dict[str, Any]) -> tuple[str, str, str, str, int, int, str]:
    return (
        str(row.get("case_id") or ""),
        str(row.get("theme") or ""),
        str(row.get("tone") or ""),
        str(row.get("background") or ""),
        int(row.get("metric_count") or 0),
        int(row.get("callout_count") or 0),
...
def _build_stage_rows(
    ...
    *,
    ...
    grouped_rows: dict[tuple[str, str, str, str, int, int, str], list[str]] = {}
    group_order: list[tuple[str, str, str, str, int, int, str]] = []
...
def _summarize_catalog(catalog_rows: list[dict[str, Any]], *, split: str) -> dict[str, Any]:
    ...
    for row in catalog_rows:
        ...
        layout_counts[str(row.get("layout") or "unknown")] += 1
...
def _split_holdout_catalog_rows(
    ...
    for row in catalog_rows:
        ...
        if not bool(row.get("training_prompt")):
            continue
        scene_id = str(row.get("case_id") or "").strip() or str(row.get("topic") or "").strip()
...
def materialize_workspace(
    ...
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"spec14a generator failed (rc={proc.returncode}):\n{proc.stdout.strip()}")
    ...
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec14a_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec14a probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec14a reset-line rerun: comparison-board only, with compositional train_aug rows plus stronger canonical-exact and hidden-style stop/compose repair rows."
            }
        }
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec14a comparison-board workspace")
    ...
    ap.add_argument("--prefix", default="spec14a_scene_dsl", help="Dataset prefix")
    ap.add_argument("--pretrain-canonical-repeat", type=int, default=48)
    ap.add_argument("--pretrain-bridge-repeat", type=int, default=12)
    ap.add_argument("--midtrain-canonical-repeat", type=int, default=36)
    ap.add_argument("--midtrain-bridge-repeat", type=int, default=12)
