#!/usr/bin/env python3
"""Materialize a cache-local spec18 routing-first bundle workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec18_v7.py"
BLUEPRINT_PATH = ROOT / "version" / "v7" / "reports" / "spec18_curriculum_blueprint.json"
LINE_NAME = "spec18_scene_bundle"
...
def materialize_workspace(
    ...
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"spec18 generator failed (rc={proc.returncode}):\n{proc.stdout.strip()}")
    ...
    if not isinstance(render_catalog_rows, list):
        raise RuntimeError("spec18 render catalog must be a JSON list")
    ...
    base.shutil.copy2(BLUEPRINT_PATH, contracts_dir / "spec18_curriculum_blueprint.json")

    probe_contract_dst = contracts_dir / f"{prefix}_probe_report_contract.json"
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec18_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec18 probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec18 keeps the frozen spec16 bundle contract and varies the prompt side by teaching routing before widening style and topology."
            }
        }
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec18 scene-bundle workspace")
