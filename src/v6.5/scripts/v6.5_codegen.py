#!/usr/bin/env python3
"""
codegen_v6.5.py - Explicit unrolled C source emitter for IR v6.5 layouts.
...
_ROOT_DIR = _SCRIPT_DIR.parents[2]  # src/v6.5/scripts -> src/v6.5 -> src -> C-Kernel-Engine
_V3_DIR = _ROOT_DIR / "scripts" / "v3"
_V6_DIR = _ROOT_DIR / "scripts" / "v6"
if _V3_DIR.is_dir():
...
# INT8 activation kernels: takes quantized input, quantized weights
# Used when --int8-activations is enabled
# Q5_0/Q8_0 use Q8_0 activations (32-element blocks)
# Q4_K uses Q8_K activations (256-element super-blocks)
...
QUANT_INFO = {
    "q8_0": {"block_size": 32, "bytes_per_block": 34, "quantize_func": "quantize_row_q8_0"},
    "q8_k": {"block_size": 256, "bytes_per_block": 292, "quantize_func": "quantize_row_q8_k"},
}


def supports_int8_activation(dtype: str) -> bool:
    """Check if dtype has a dedicated INT8 activation kernel."""
    qt = get_quant_type(dtype)
    return qt in DTYPE_TO_GEMV_Q8_KERNEL


def get_int8_kernel_info(dtype: str) -> tuple:
...
def quant_bytes_for_elements(n: str, act_type: str) -> str:
    """Return C expression for quantized buffer size in bytes.

    Q8_0 format: 32 elements per block, 34 bytes per block (2 byte scale + 32 int8 values).
    Q8_K format: 256 elements per block, 292 bytes per block (super-block with scales).
    """
    info = QUANT_INFO.get(act_type)
    if not info:
        return f"(((({n}) + 31) / 32) * 34)"  # Default to Q8_0
    block_size = info["block_size"]
    bytes_per_block = info["bytes_per_block"]
    return f"(((({n}) + {block_size - 1}) / {block_size}) * {bytes_per_block})"


def get_quantize_func(act_type: str) -> str:
    """Get quantization function name for activation type."""
    info = QUANT_INFO.get(act_type)
    return info["quantize_func"] if info else "quantize_row_q8_0"
...
def emit_c_source_v65(layout: v3.ModelLayout,
                     output_path: str,
                     header_name: str,
                     mode: str,
    ...
    if mode not in ("prefill", "decode"):
        raise ValueError(f"v6.5 codegen only supports prefill/decode (got: {mode})")
    ...
    add(f" * @brief AUTO-GENERATED: {layout.name} Implementation (IR v6.5 - Explicit Unrolled)")
    add(f" *")
    ...
    add(f" *")
    add(f" * DO NOT EDIT - Regenerate with build_ir_v6.5.py or codegen_v6.5.py")
    add(f" */")
    add()
    ...
    add(f'#error "{layout.name}: v6.5 codegen currently supports fp32 only. Use --dtype=fp32."')
    add("#endif")
    add()
    ...
    def emit_prefill_impl() -> None:
        add("/* ============================================================================")
        add(" * EXPLICIT PER-LAYER PREFILL FUNCTIONS (v6.5 unrolled)")
        add(" * ============================================================================ */")
        add()
        ...
        for layer_id in range(num_layers):
            ...
            # Check if INT8 activations should be used for Q projection
            use_int8_q = int8_activations and supports_int8_activation(wq_dt)
            add("    /* Q projection (head-major) */")
            if use_int8_q:
                wq_kernel_int8, wq_act_type = get_int8_kernel_info(wq_dt)
                wq_quant_func = get_quantize_func(wq_act_type)
                add(f"    /* INT8 path: {wq_dt.upper()} x {wq_act_type.upper()} -> {wq_kernel_int8} (row-by-row) */")
                add(f"    const size_t ln1_q8_bytes = {quant_bytes_for_elements('aligned_embed_dim', wq_act_type)};")
                ...
                add("        const float *ln1_row = ln1_out + (size_t)t * (size_t)aligned_embed_dim;")
                add("        uint8_t ln1_q8[ln1_q8_bytes];")
                add(f"        {wq_quant_func}(ln1_row, ln1_q8, aligned_embed_dim);")
                add("        for (int h = 0; h < H; ++h) {")
                add("            const void *wq_h = (const void *)(WQ_bytes + (size_t)h * wq_head_bytes);")
                add("            float *q_out = q + (size_t)h * q_head_stride + (size_t)t * (size_t)aligned_head_dim;")
                add(f"            {wq_kernel_int8}(q_out, wq_h, ln1_q8, head_dim, aligned_embed_dim);")
                if has_attention_biases:
                    add("            const float *bq_h = BQ ? (BQ + (size_t)h * (size_t)aligned_head_dim) : NULL;")
                    add("            if (bq_h) { for (int d = 0; d < head_dim; ++d) q_out[d] += bq_h[d]; }")
                add("        }")
                add("    }")
            elif wq_dt == "fp32":
            ...
            # Check if INT8 activations should be used for K projection
            use_int8_k = int8_activations and supports_int8_activation(wk_dt)
            add("    /* K projection (head-major) */")
            if use_int8_k:
                wk_kernel_int8, wk_act_type = get_int8_kernel_info(wk_dt)
                wk_quant_func = get_quantize_func(wk_act_type)
                add(f"    /* INT8 path: {wk_dt.upper()} x {wk_act_type.upper()} -> {wk_kernel_int8} (row-by-row to KV cache) */")
                # Reuse ln1_q8 if same as Q, otherwise declare new
                if not use_int8_q:
                    add(f"    const size_t ln1_q8_bytes_k = {quant_bytes_for_elements('aligned_embed_dim', wk_act_type)};")
                ...
                add("    for (int t = 0; t < num_tokens; ++t) {")
                add("        const float *ln1_row_k = ln1_out + (size_t)t * (size_t)aligned_embed_dim;")
                if not use_int8_q:
                    add("        uint8_t ln1_q8_k[ln1_q8_bytes_k];")
                    add(f"        {wk_quant_func}(ln1_row_k, ln1_q8_k, aligned_embed_dim);")
                else:
                    add("        uint8_t ln1_q8_k[ln1_q8_bytes];")
                    add(f"        {wk_quant_func}(ln1_row_k, ln1_q8_k, aligned_embed_dim);")
                add("        for (int h = 0; h < H_kv; ++h) {")
                add("            const void *wk_h = (const void *)(WK_bytes + (size_t)h * wk_head_bytes);")
                add("            float *k_out = k + (size_t)h * kv_head_stride + (size_t)t * (size_t)aligned_head_dim;")
                add(f"            {wk_kernel_int8}(k_out, wk_h, ln1_q8_k, head_dim, aligned_embed_dim);")
                if has_attention_biases:
                    add("            const float *bk_h = BK ? (BK + (size_t)h * (size_t)aligned_head_dim) : NULL;")
                    add("            if (bk_h) { for (int d = 0; d < head_dim; ++d) k_out[d] += bk_h[d]; }")
                add("        }")
                add("    }")
            elif wk_dt == "fp32":
            ...
            # Check if INT8 activations should be used for V projection
            use_int8_v = int8_activations and supports_int8_activation(wv_dt)
            add("    /* V projection (head-major) */")
            if use_int8_v:
                wv_kernel_int8, wv_act_type = get_int8_kernel_info(wv_dt)
                wv_quant_func = get_quantize_func(wv_act_type)
                add(f"    /* INT8 path: {wv_dt.upper()} x {wv_act_type.upper()} -> {wv_kernel_int8} (row-by-row to KV cache) */")
                # Declare buffer size if not already defined
                if not use_int8_q and not use_int8_k:
                    add(f"    const size_t ln1_q8_bytes_v = {quant_bytes_for_elements('aligned_embed_dim', wv_act_type)};")
                ...
                add("    for (int t = 0; t < num_tokens; ++t) {")
                add("        const float *ln1_row_v = ln1_out + (size_t)t * (size_t)aligned_embed_dim;")
                if use_int8_q:
                    add("        uint8_t ln1_q8_v[ln1_q8_bytes];")
                elif use_int8_k:
                    add("        uint8_t ln1_q8_v[ln1_q8_bytes_k];")
                else:
                    add("        uint8_t ln1_q8_v[ln1_q8_bytes_v];")
                add(f"        {wv_quant_func}(ln1_row_v, ln1_q8_v, aligned_embed_dim);")
                add("        for (int h = 0; h < H_kv; ++h) {")
                add("            const void *wv_h = (const void *)(WV_bytes + (size_t)h * wv_head_bytes);")
                add("            float *v_out = v + (size_t)h * kv_head_stride + (size_t)t * (size_t)aligned_head_dim;")
                add(f"            {wv_kernel_int8}(v_out, wv_h, ln1_q8_v, head_dim, aligned_embed_dim);")
                if has_attention_biases:
                    add("            const float *bv_h = BV ? (BV + (size_t)h * (size_t)aligned_head_dim) : NULL;")
                    add("            if (bv_h) { for (int d = 0; d < head_dim; ++d) v_out[d] += bv_h[d]; }")
                add("        }")
                add("    }")
            elif wv_dt == "fp32":
            ...
            # Check if INT8 activations should be used for WO projection
            ...
            if use_int8_wo:
                ...
                add(f"    /* INT8 output projection: {wo_dt.upper()} x {wo_act_type.upper()} -> {wo_kernel_int8} (row-by-row) */")
                add(f"    const size_t proj_q8_bytes = {quant_bytes_for_elements('K', wo_act_type)};")
                add("    for (int t = 0; t < num_tokens; ++t) {")
                add("        const float *proj_row = proj_in + (size_t)t * (size_t)K;")
                add("        float *out_row = proj_tmp + (size_t)t * (size_t)aligned_embed_dim;")
                add("        uint8_t proj_q8[proj_q8_bytes];")
                add(f"        {wo_quant_func}(proj_row, proj_q8, K);")
                add(f"        {wo_kernel_int8}(out_row, WO, proj_q8, aligned_embed_dim, K);")
                if has_output_bias:
                    add("        if (BO) { for (int d = 0; d < aligned_embed_dim; ++d) out_row[d] += BO[d]; }")
                add("    }")
            else:
            ...
            add("    /* MLP (SwiGLU) */")
            # Check if INT8 activations should be used for W1
            ...
            if use_int8_w1:
                ...
                add(f"    /* INT8 W1: {w1_dt.upper()} x {w1_act_type.upper()} -> {w1_kernel_int8} (row-by-row) */")
                add(f"    const size_t ln2_q8_bytes = {quant_bytes_for_elements('aligned_embed_dim', w1_act_type)};")
                add("    for (int t = 0; t < num_tokens; ++t) {")
                add("        const float *ln2_row = ln2_out + (size_t)t * (size_t)aligned_embed_dim;")
                add("        float *fc1_row = fc1_out + (size_t)t * (size_t)(2 * aligned_intermediate_dim);")
                add("        uint8_t ln2_q8[ln2_q8_bytes];")
                add(f"        {w1_quant_func}(ln2_row, ln2_q8, aligned_embed_dim);")
                add(f"        {w1_kernel_int8}(fc1_row, W1, ln2_q8, 2 * aligned_intermediate_dim, aligned_embed_dim);")
                if has_mlp_bias1:
                    add("        if (B1) { for (int d = 0; d < 2 * aligned_intermediate_dim; ++d) fc1_row[d] += B1[d]; }")
                add("    }")
            else:
            ...
            # Check if INT8 activations should be used for W2
            ...
            if use_int8_w2:
                ...
                add(f"    /* INT8 W2: {w2_dt.upper()} x {w2_act_type.upper()} -> {w2_kernel_int8} (row-by-row) */")
                add(f"    const size_t swiglu_q8_bytes = {quant_bytes_for_elements('aligned_intermediate_dim', w2_act_type)};")
                add("    for (int t = 0; t < num_tokens; ++t) {")
                add("        const float *swiglu_row = swiglu_out + (size_t)t * (size_t)aligned_intermediate_dim;")
                add("        float *mlp_row = mlp_out + (size_t)t * (size_t)aligned_embed_dim;")
                add("        uint8_t swiglu_q8[swiglu_q8_bytes];")
                add(f"        {w2_quant_func}(swiglu_row, swiglu_q8, aligned_intermediate_dim);")
                add(f"        {w2_kernel_int8}(mlp_row, W2, swiglu_q8, aligned_embed_dim, aligned_intermediate_dim);")
                if has_mlp_bias2:
                    add("        if (B2) { for (int d = 0; d < aligned_embed_dim; ++d) mlp_row[d] += B2[d]; }")
                add("    }")
            else:
        ...
        if lm_head_quant_type != "fp32":
            ...
            if lm_head_use_q4_k:
                ...
                # Check if INT8 activations can be used for non-Q4_K quant types
                use_int8_lm_head = int8_activations and supports_int8_activation(lm_head_quant_type)
                if use_int8_lm_head:
                    lm_kernel_int8, lm_act_type = get_int8_kernel_info(lm_head_quant_type)
                    lm_quant_func = get_quantize_func(lm_act_type)
                    add(f"    /* INT8 LM head: {lm_head_quant_type.upper()} x {lm_act_type.upper()} -> {lm_kernel_int8} (row-by-row) */")
                    add(f"    const size_t final_q8_bytes = {quant_bytes_for_elements('aligned_embed_dim', lm_act_type)};")
                    add("    for (int t = 0; t < num_tokens; ++t) {")
                    add("        const float *row = final_out + (size_t)t * (size_t)aligned_embed_dim;")
                    add(f"        float *logits_row = logits + (size_t)t * (size_t){safe_name}_VOCAB_SIZE;")
                    add("        uint8_t final_q8[final_q8_bytes];")
                    add(f"        {lm_quant_func}(row, final_q8, aligned_embed_dim);")
                    add(f"        {lm_kernel_int8}(logits_row, lm_head, final_q8, {safe_name}_VOCAB_SIZE, aligned_embed_dim);")
                    add("    }")
                else:
                    add("    for (int t = 0; t < num_tokens; ++t) {")
                    add("        const float *row = final_out + (size_t)t * (size_t)aligned_embed_dim;")
    ...
    if mode == "decode":
        add("/* ============================================================================")
        add(" * EXPLICIT PER-LAYER DECODE FUNCTIONS (v6.5 unrolled)")
        add(" * ============================================================================ */")
        add()
        ...
        for layer_id in range(num_layers):
            ...
            # INT8 activation path: quantize FP32 -> Q8_0, use gemv_q*_q8_0
            use_int8_qkv = int8_activations and supports_int8_activation(wq_dt)
            if use_int8_qkv:
                wq_kernel_int8, wq_act_type = get_int8_kernel_info(wq_dt)
                wq_quant_func = get_quantize_func(wq_act_type)
                add(f"    /* Q projection (INT8): {wq_dt.upper()} x {wq_act_type.upper()} -> {wq_kernel_int8} */")
                add(f"    const size_t ln1_q8_bytes = {quant_bytes_for_elements('aligned_embed_dim', wq_act_type)};")
                add("    uint8_t ln1_q8[ln1_q8_bytes];")
                add(f"    {wq_quant_func}(ln1_out, ln1_q8, aligned_embed_dim);")
                add(f"    {wq_kernel_int8}(q_token, WQ, ln1_q8, H * head_dim, aligned_embed_dim);")
                # Handle bias manually if needed
                if has_attention_biases:
                    add("    if (BQ) {")
            ...
            if use_int8_k:
                wk_kernel_int8, wk_act_type = get_int8_kernel_info(wk_dt)
                add(f"    /* K projection (INT8): {wk_dt.upper()} x {wk_act_type.upper()} -> {wk_kernel_int8} (direct-to-cache) */")
                ...
                add("    const uint8_t *WK_bytes = (const uint8_t *)WK;")
                add("    /* ln1_q8 already quantized above */")
                add("    for (int h = 0; h < H_kv; ++h) {")
                add("        const void *wk_h = (const void *)(WK_bytes + (size_t)h * wk_head_bytes);")
                add("        float *k_head = k_cache + (size_t)h * kv_head_stride + (size_t)token_index * (size_t)aligned_head_dim;")
                add(f"        {wk_kernel_int8}(k_head, wk_h, ln1_q8, head_dim, aligned_embed_dim);")
                if has_attention_biases:
                    add("        const float *bk_h = BK ? (BK + (size_t)h * (size_t)aligned_head_dim) : NULL;")
            ...
            if use_int8_v:
                wv_kernel_int8, wv_act_type = get_int8_kernel_info(wv_dt)
                add(f"    /* V projection (INT8): {wv_dt.upper()} x {wv_act_type.upper()} -> {wv_kernel_int8} (direct-to-cache) */")
                ...
                add(f"        {wv_kernel_int8}(v_head, wv_h, ln1_q8, head_dim, aligned_embed_dim);")
                if has_attention_biases:
                    add("        const float *bv_h = BV ? (BV + (size_t)h * (size_t)aligned_head_dim) : NULL;")
        ...
        use_int8_lm_head = int8_activations and supports_int8_activation(lm_head_quant_type)
        if use_int8_lm_head:
            lm_head_kernel_int8, lm_head_act_type = get_int8_kernel_info(lm_head_quant_type)
            lm_head_quant_func = get_quantize_func(lm_head_act_type)
            add(f"    const void *lm_head = (const void *){safe_name}_PTR(model, {safe_name}_FOOTER.lm_head_weight);")
            add(f"    /* LM head (INT8): {lm_head_quant_type.upper()} x {lm_head_act_type.upper()} -> {lm_head_kernel_int8} */")
            add(f"    const size_t final_q8_bytes = {quant_bytes_for_elements('aligned_embed_dim', lm_head_act_type)};")
            add("    uint8_t final_q8[final_q8_bytes];")
            add(f"    {lm_head_quant_func}(final_out, final_q8, aligned_embed_dim);")
            add(f"    {lm_head_kernel_int8}(logits, lm_head, final_q8, {safe_name}_VOCAB_SIZE, aligned_embed_dim);")
        elif lm_head_quant_type != "fp32":
            add(f"    const void *lm_head = (const void *){safe_name}_PTR(model, {safe_name}_FOOTER.lm_head_weight);")
        ...
        # =========================================================================
        add("/* ============================================================================")
        add(" * GENERIC MODEL API - Model-agnostic interface")
        add(" * These functions have the SAME names for all models.")
        add(" * CLI code uses these, so it works with any model.")
        add(" * ============================================================================ */")
        add()
        add('#include "ck_model_api.h"')
        add()

        # Static config
        add(f"static CKModelConfig g_model_config = {{")
        add(f"    .embed_dim = {safe_name}_EMBED_DIM,")
        add(f"    .num_heads = {safe_name}_NUM_HEADS,")
        add(f"    .num_kv_heads = {safe_name}_NUM_KV_HEADS,")
        add(f"    .head_dim = {safe_name}_HEAD_DIM,")
        add(f"    .intermediate_size = {safe_name}_INTERMEDIATE,")
        add(f"    .num_layers = {safe_name}_NUM_LAYERS,")
        add(f"    .vocab_size = {safe_name}_VOCAB_SIZE,")
        add(f"    .max_seq_len = {safe_name}_MAX_SEQ_LEN,")
        add(f"    .total_bytes = {safe_name}_TOTAL_BYTES,")
        add(f"    .weight_bytes = {safe_name}_WEIGHT_BYTES,")
        add(f"    .activation_bytes = {safe_name}_ACTIVATION_BYTES,")
        add(f'    .model_name = "{layout.name}",')
        add(f'    .model_family = "{layout.name.split("_")[0]}",')
        add("};")
        add()

        # Generic API functions
        add("const CKModelConfig *ck_model_get_config(void) {")
        add("    return &g_model_config;")
        add("}")
        add()

        add("void *ck_model_create(void) {")
        add(f"    {safe_name}Model *model = malloc(sizeof({safe_name}Model));")
        add("    if (!model) return NULL;")
        ...
        add("void ck_model_free(void *model) {")
        add("    if (!model) return;")
        add(f"    {safe_name_lower}_model_free(({safe_name}Model *)model);")
        add("    free(model);")
        add("}")
        add()

        add("void ck_model_precompute_rope(void *model) {")
        add(f"    {safe_name_lower}_precompute_rope(({safe_name}Model *)model);")
        add("}")
        add()

        add("void ck_model_forward(void *model, const int *tokens, int num_tokens) {")
        add(f"    {safe_name_lower}_forward(({safe_name}Model *)model, tokens, num_tokens);")
        add("}")
        add()

        add("void ck_model_decode(void *model, const int *token, int token_index) {")
        add(f"    {safe_name_lower}_decode(({safe_name}Model *)model, token, token_index);")
        add("}")
        add()

        add("float *ck_model_get_logits(void *model) {")
        add(f"    {safe_name}Model *m = ({safe_name}Model *)model;")
        add(f"    return {safe_name}_PTR(m, {safe_name}_FOOTER.logits);")
        add("}")
        add()

        add("int ck_model_verify_canaries(void *model) {")
        add(f"    return {safe_name_lower}_verify_canaries(({safe_name}Model *)model);")
        add("}")
        add()

        add("void *ck_model_get_base(void *model) {")
        add(f"    return (({safe_name}Model *)model)->base;")
        add("}")
        add()

        add("size_t ck_model_get_total_bytes(void *model) {")
        add(f"    return (({safe_name}Model *)model)->total_bytes;")
        add("}")
        add()
...
if __name__ == "__main__":
    ...
    # INT8 activation control
    # Default: auto (INT8 for decode/inference, FP32 for training/backward)
    parser.add_argument("--activations", choices=["auto", "int8", "fp32"], default="auto",
                        help="Activation precision: auto (default), int8 (fast), fp32 (precise)")
    ...
    # Determine INT8 activation setting
    # Priority: explicit --activations > legacy flags > auto (mode-based default)
    if args.activations == "int8" or args.int8_activations:
        use_int8 = True
    elif args.activations == "fp32" or args.no_int8_activations:
        use_int8 = False
    else:  # auto
        # Default: INT8 for ALL inference modes (prefill + decode)
        # Only training/backward would use FP32 (not implemented yet)
        use_int8 = args.mode in ("decode", "prefill")

    if use_int8:
        print(f"[CODEGEN] Using INT8 activations (Q8_K/Q8_0) for faster inference")
    else:
        print(f"[CODEGEN] Using FP32 activations")

    # Load lowered IR and build layout
    with open(args.lowered) as f:
        lowered = json.load(f)

    layout = v6.build_layout_from_lowered(lowered, args.model_name)
