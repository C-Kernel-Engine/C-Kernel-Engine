#!/usr/bin/env python3
"""
Trace v7 execution and localize first failing stop-op.

Usage:
    python trace_divergence.py --model ~/.cache/ck-engine-v7/models/<model> --token 25
"""
...
DEFAULT_V7_DIR = Path.home() / ".cache/ck-engine-v7/models/Qwen--Qwen2-0.5B-Instruct-GGUF"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Trace v7 stop-op execution")
    parser.add_argument("--model", type=Path, default=DEFAULT_V7_DIR, help="Path to v7 model directory")
    ...
    print("=" * 70)
    print(f"TRACING V7 EXECUTION for token {args.token}")
    print("=" * 70)
    ...
    try:
        ...
        print(f"ERROR: v7 decode failed: {e}")
        return 1
    ...
    print(
        f"v7 logits: min={logits.min():.4f}, max={logits.max():.4f}, "
        f"argmax={int(np.argmax(logits))}"
    )

    print("\n--- V7 ACTIVATION SUMMARY ---")
    for key in (
        "embedding",
        "layer_input",
        "residual",
    ...
    if not args.skip_stop_scan:
        print("\n" + "=" * 70)
        print("STEP-BY-STEP STOP_OP TEST (v7)")
        print("=" * 70)
