...
RUN="${1:-/tmp/spec05_structured_scenes_l3_d192_h384_ctx128}"
DATASET_PREFIX="${DATASET_PREFIX:-spec05_structured_svg_atoms}"
...
PRETRAIN_TOTAL_TOKENS="${PRETRAIN_TOTAL_TOKENS:-786432}"
MIDTRAIN_TOTAL_TOKENS="${MIDTRAIN_TOTAL_TOKENS:-1048576}"
...
PROBE_CONTRACT="$RUN/spec05_probe_contract.json"

cd "$ROOT"
mkdir -p "$RUN"

python3 version/v7/scripts/dataset/materialize_spec05_structured_atoms_v7.py \
  --workspace "$WORKSPACE" \
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec05_pretrain_midtrain_v7.sh",
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 3e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec05_stage_a.json"
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 2.5e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec05_stage_b.json"
...
if [ "$RUN_STAGE_EVAL" = "1" ]; then
  ...
  echo "[stage-eval] skipped builtin default probes; use spec05_probe_report.json/html for structured-scene evaluation."
fi

python3 version/v7/scripts/build_structured_scene_probe_contract_v7.py \
  --run "$RUN" \
  --prefix "$DATASET_PREFIX" \
...
python3 version/v7/scripts/build_probe_report_v7.py \
  ...
  --output "$RUN/spec05_probe_report.html" \
  --json-out "$RUN/spec05_probe_report.json"

python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
  --probe-report "$RUN/spec05_probe_report.json" \
  --output-html "$RUN/spec05_tested_prompts_report.html" \
  --output-md "$RUN/spec05_tested_prompts_report.md"
...
printf 'spec05 pretrain+midtrain finished for %s\n' "$RUN"
