...
import build_spec17_probe_contract_v7 as probe_contract  # type: ignore
import generate_svg_structured_spec17_v7 as spec17_generator  # type: ignore
import materialize_spec17_scene_bundle_v7 as materializer  # type: ignore
...
class Spec17CurriculumContractsTests(unittest.TestCase):
    def test_profile_cases_cover_three_families_and_nine_profiles(self) -> None:
        cases = spec17_generator._build_profile_cases()
    ...
    def test_prompt_rows_emit_declared_bridge_scaffolds(self) -> None:
        case = spec17_generator._build_profile_cases()[0]
        surface_ids = {surface for _, surface, _, _ in spec17_generator._prompt_rows(case)}

        self.assertIn("explicit_permuted_anchor", surface_ids)
        self.assertIn("layout_topic_bridge", surface_ids)
        self.assertIn("layout_form_bridge", surface_ids)
        self.assertIn("layout_hint_bridge", surface_ids)

    def test_hidden_probe_picker_balances_across_families(self) -> None:
        prompts: list[str] = []
        catalog: dict[str, dict[str, str | bool]] = {}
        for family, form_token in (
            ("memory_map", "layer_stack"),
            ("timeline", "stage_sequence"),
            ("system_diagram", "build_path"),
        ):
            for idx in range(3):
                case_id = f"{family}_{idx}"
                for surface in ("hidden_paraphrase", "hidden_recombination"):
                    prompt = f"[task:{family}:{idx}:{surface}] [OUT]"
    ...
    def test_form_contrast_prompts_encode_sibling_form_boundary(self) -> None:
        ...
        for case in cases:
            prompt_rows = {
                surface: prompt
                for prompt, surface, _, _ in spec17_generator._prompt_rows(case, lookup)
            }
            prompt = prompt_rows["form_disambiguation_contrast"]
            tags = _parse_prompt_tags(prompt)
            sibling = lookup[spec17_generator.FORM_CONTRAST_PARTNER_IDS[case.profile_id]]
    ...
    def test_family_contrast_prompts_encode_cross_family_boundary(self) -> None:
        ...
        for case in cases:
            ...
            prompt = prompt_rows["family_confusion_contrast"]
            tags = _parse_prompt_tags(prompt)
            sibling = lookup[spec17_generator.FAMILY_CONTRAST_PARTNER_IDS[case.profile_id]]
    ...
    def test_stage_rows_follow_blueprint_surfaces_without_duplicate_backing(self) -> None:
        ...
        self.assertIn("explicit_permuted_anchor", prompt_surfaces)
        self.assertIn("layout_topic_bridge", prompt_surfaces)
        self.assertIn("layout_form_bridge", prompt_surfaces)
        self.assertIn("layout_hint_bridge", prompt_surfaces)
        self.assertIn("intent_family_bridge", prompt_surfaces)
        self.assertIn("topic_goal_recombination", prompt_surfaces)
        self.assertIn("paraphrase_bridge", prompt_surfaces)
        self.assertIn("family_confusion_contrast", prompt_surfaces)

    def test_frozen_tokenizer_copy_strips_placeholder_special_token_but_keeps_vocab_slot(self) -> None:
        with tempfile.TemporaryDirectory(prefix="spec17_tok_") as tmp:
            root = Path(tmp)
            ...
            reserved = (root / "out_tokenizer" / "spec17_scene_bundle_reserved_control_tokens.txt").read_text(encoding="utf-8").splitlines()
