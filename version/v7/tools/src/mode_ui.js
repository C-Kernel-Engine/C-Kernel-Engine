import { ensurePanel, getEmbeddedFiles, tabIdForElement } from './utils.js';
...
function detectTrainingModeAvailable() {
    const embedded = window.EMBEDDED_IR_DATA && typeof window.EMBEDDED_IR_DATA === 'object'
        ? window.EMBEDDED_IR_DATA
        : {};
    const meta = embedded.meta && typeof embedded.meta === 'object' ? embedded.meta : {};
    if (meta.has_train_runtime === true) return true;

    const files = getEmbeddedFiles();
    return TRAINING_SIGNAL_KEYS.some((key) => {
        const payload = files[key];
...
function injectStyles() {
    ...
    style.textContent = `
        ...
        .report-mode-toggle button:disabled {
            opacity: 0.45;
            cursor: not-allowed;
            background: var(--dark-card);
...
function ensureTrainingPanels() {
    ...
    ensurePanel(
        'train-data-lab',
        'Data Lab',
        'Dataset quality, tokenizer roundtrip, and output quality evaluation.'
    );
    ensurePanel(
        'train-gradient',
        'Gradient Health',
...
function installModeToggle() {
    ...
    `;
    const trainingBtn = host.querySelector('button[data-mode="training"]');
    if (trainingBtn && window.__ckTrainingModeAvailable === false) {
        trainingBtn.disabled = true;
        trainingBtn.title = 'Training mode unavailable: this report was generated from inference-only artifacts.';
    }
...
export function applyReportMode(mode) {
    let normalized = mode === 'training' ? 'training' : 'inference';
    if (normalized === 'training' && window.__ckTrainingModeAvailable === false) {
