#!/usr/bin/env python3
"""Preflight gates for spec17 bounded-intent scene-bundle training."""
...
from build_spec17_probe_contract_v7 import build_contract as build_probe_contract
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec17 render catalog must be a JSON list")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Preflight gates for spec17 bounded-intent scene-bundle training")
