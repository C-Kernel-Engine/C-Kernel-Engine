#!/usr/bin/env python3
"""Materialize a cache-local spec13b graph-family workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec13b_v7.py"
...
def _row_repeat(prompt_surface: str, *, canonical_repeat: int, bridge_repeat: int) -> int:
    repeat = int(canonical_repeat) if str(prompt_surface or "") == "tag_canonical" else int(bridge_repeat)
    return max(1, repeat)
...
def _build_stage_rows(
    ...
    *,
    ...
    for row in catalog_rows:
        ...
        if str(row.get("layout") or "") == "decision_tree":
            repeat += max(0, int(decision_bonus_repeat))
...
def _summarize_catalog(catalog_rows: list[dict[str, Any]], *, split: str) -> dict[str, Any]:
    ...
    for row in catalog_rows:
        if not isinstance(row, dict) or str(row.get("split") or "") != split or not bool(row.get("training_prompt")):
            continue
...
def materialize_workspace(
    ...
    *,
    ...
    pretrain_decision_bonus_repeat: int,
    midtrain_decision_bonus_repeat: int,
    python_exec: str,
    force: bool,
) -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"spec13b generator failed (rc={proc.returncode}):\n{proc.stdout.strip()}")
    ...
    pretrain_train_rows = _build_stage_rows(
        ...
        decision_bonus_repeat=int(pretrain_decision_bonus_repeat),
        base=base,
    )
    midtrain_train_rows = _build_stage_rows(
        ...
        decision_bonus_repeat=int(midtrain_decision_bonus_repeat),
        base=base,
    )
    ...
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec13b_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec13b probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    workspace_manifest = {
        ...
        "pretrain_decision_bonus_repeat": int(pretrain_decision_bonus_repeat),
        "midtrain_decision_bonus_repeat": int(midtrain_decision_bonus_repeat),
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec13b r1 is graph-family only. Whole-scene canonical + bridge rows only. No fragment repair rows are added.",
            },
        },
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec13b graph-family workspace")
    ...
    ap.add_argument("--prefix", default="spec13b_scene_dsl", help="Dataset prefix")
    ap.add_argument("--family-mode", choices=("all", "decision_tree", "flow_graph"), default="all")
    ap.add_argument("--pretrain-canonical-repeat", type=int, default=32)
    ap.add_argument("--pretrain-bridge-repeat", type=int, default=12)
    ap.add_argument("--midtrain-canonical-repeat", type=int, default=24)
    ap.add_argument("--midtrain-bridge-repeat", type=int, default=12)
    ap.add_argument("--pretrain-decision-bonus-repeat", type=int, default=0)
    ap.add_argument("--midtrain-decision-bonus-repeat", type=int, default=0)
    ...
    summary = materialize_workspace(
        ...
        pretrain_decision_bonus_repeat=int(args.pretrain_decision_bonus_repeat),
        midtrain_decision_bonus_repeat=int(args.midtrain_decision_bonus_repeat),
