#!/usr/bin/env python3
"""
test_numerical_parity.py - Numerical validation for v7 runtime only.

USAGE:
    python test_numerical_parity.py --model ~/.cache/ck-engine-v7/models/<model>
    python test_numerical_parity.py --model <path> --tokens 1 25 42 --threads 1 --context-len 1024

This test:
1. Runs input tokens through v7 decode API
...
@dataclass
class LayerSnapshot:
    """Snapshot of an observed tensor in the v7 pipeline."""
...
@dataclass
class PipelineTrace:
    """Minimal execution trace for a v7 decode run."""
...
class NumericalValidationTest:
    """Numerical health check for v7 output."""
    ...
    def _load_v7_library(self) -> ctypes.CDLL:
        """Load generated v7 model library."""
    ...
    def run_v7(self, input_tokens: List[int]) -> PipelineTrace:
        """Run v7 decode path and capture final logits."""
        trace = PipelineTrace(model_name="v7", input_tokens=input_tokens)

        os.environ["CK_NUM_THREADS"] = str(self.threads)
        os.environ["OMP_NUM_THREADS"] = str(self.threads)

        lib = self._load_v7_library()
        weights_path = self.model_path / "weights.bump"

        result = lib.ck_model_init(str(weights_path).encode())
        if result != 0:
            raise RuntimeError(f"v7 init failed: {result}")
        ...
        for i, token in enumerate(input_tokens):
            ...
            if result != 0:
                raise RuntimeError(f"v7 decode failed at token index {i}: {result}")
    ...
    def run_test(self, input_tokens: Optional[List[int]] = None) -> Dict:
        """Run v7 numerical validation."""
        if input_tokens is None:
            input_tokens = [1]

        print("=" * 70)
        print("NUMERICAL VALIDATION TEST: v7")
        print("=" * 70)
        ...
        print("Running v7...")
        trace = self.run_v7(input_tokens)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Numerical validation test for v7")
    parser.add_argument("--model", type=Path, help="Path to v7 model directory")
