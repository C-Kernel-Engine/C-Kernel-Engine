#!/usr/bin/env python3
"""
test_memory_planner.py - Memory Layout Quick Validator for v7
...
class MemoryPlannerTest:
    ...
    def validate_layer_stride(self) -> bool:
        """Validate layer offsets - variable stride is OK in v7."""
        ...
        # v7 uses variable stride due to mixed quantization
        if len(unique_strides) > 1:
            self.result.warnings.append(
                f"Variable layer stride detected ({len(unique_strides)} unique values): "
                f"{sorted(unique_strides)[:5]}... This is expected in v7 with mixed quantization"
            )
