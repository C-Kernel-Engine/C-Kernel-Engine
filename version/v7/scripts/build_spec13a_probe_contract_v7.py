#!/usr/bin/env python3
"""Build a balanced probe contract for spec13a intent-prompt runs."""
...
def _case_from_prompt(prompt: str, catalog: dict[str, dict[str, Any]], split: str, split_label: str, index: int) -> dict[str, Any]:
    ...
    prompt_family = str(row.get("prompt_family") or "prompt")
    return {
        "id": f"{split}_{index:02d}",
        "label": f"{split_label} {_label_prefix(layout)} #{index} ({prompt_family})",
        "prompt": prompt,
