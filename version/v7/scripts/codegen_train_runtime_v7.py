...
def generate_c(ir2: Dict[str, Any], registry: Dict[str, Any], manifest: Optional[Dict[str, Any]] = None, layout: Optional[Dict[str, Any]] = None, exec_plan: Optional[Dict[str, Any]] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    ap("#endif")
    ap("#ifndef CK_ADAMW_BETA1")
    ap("#define CK_ADAMW_BETA1 0.9f")
    ap("#endif")
    ap("#ifndef CK_ADAMW_BETA2")
    ap("#define CK_ADAMW_BETA2 0.999f")
    ap("#endif")
    ap("#ifndef CK_ADAMW_EPS")
    ap("#define CK_ADAMW_EPS 1e-8f")
    ap("#endif")
    ap("#ifndef CK_ADAMW_WEIGHT_DECAY")
    ap("#define CK_ADAMW_WEIGHT_DECAY 0.01f")
    ap("#endif")
    ...
    ap("static float g_last_grad_norm = 0.0f;")
    ...
    ap("#if defined(TIME_UTC)")
    ap("    struct timespec ts;")
    ap("    if (timespec_get(&ts, TIME_UTC) == TIME_UTC) {")
    ap("        return (double)ts.tv_sec * 1000.0 + (double)ts.tv_nsec / 1000000.0;")
    ap("    }")
    ap("#endif")
    ap("    return (double)clock() * 1000.0 / (double)CLOCKS_PER_SEC;")
    ap("}")
    ap("")
    ap("static float ck_train_compute_global_grad_norm(void) {")
    if not opt_pairs:
        ap("    return 0.0f;")
    else:
        ap("    /*")
    ...
    ap("int ck_train_get_last_step_grad_norm(float *grad_norm);")
    ...
    if "adamw_update_f32" not in used_decl:
        ...
        ap("    const float beta1 = (float)CK_ADAMW_BETA1;")
        ap("    const float beta2 = (float)CK_ADAMW_BETA2;")
        ap("    const float eps = (float)CK_ADAMW_EPS;")
    ...
    ap("}")
    ap("int ck_train_get_last_step_grad_norm(float *grad_norm) {")
    ap("    if (grad_norm != NULL) *grad_norm = g_last_grad_norm;")
    ap("    return 0;")
    ap("}")
    ap("int ck_train_get_cumulative_profile(double *total_ms, double *fwd_ms, double *bwd_ms, double *opt_ms, int *steps, int *optimizer_steps) {")
