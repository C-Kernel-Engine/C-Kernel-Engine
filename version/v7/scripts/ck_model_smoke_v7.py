#!/usr/bin/env python3
"""
ck_model_smoke_v7.py
====================

Lightweight v7 runtime smoke test:
  - initialize model
...
def main() -> int:
    ap = argparse.ArgumentParser(description="v7 runtime smoke test")
    ap.add_argument("--model-dir", required=True, help="Directory with ck-kernel-inference.so")
    ...
    if not lib_path.exists():
        lib_path = model_dir / "ck-kernel-decode.so"
    if not lib_path.exists():
        lib_path = model_dir / "libmodel.so"
    if not lib_path.exists():
        print(f"Error: model library not found at {model_dir}", file=sys.stderr)
        return 1
