...
export SPEC_TAG="${SPEC_TAG:-spec19r3f}"
...
export RUN_KIND="${RUN_KIND:-seeded_spec19_cumulative_balanced_route_recovery_continuation}"

export RUN_SCOPE_SPEC="${RUN_SCOPE_SPEC:-spec19}"
export RUN_SCOPE_RUNG="${RUN_SCOPE_RUNG:-r3f}"
export RUN_SCOPE_FAMILY="${RUN_SCOPE_FAMILY:-visual_scene_bundle}"
export RUN_SCOPE_TITLE="${RUN_SCOPE_TITLE:-Spec19 R3f Cumulative Balanced Route Recovery Continuation}"
export RUN_SCOPE_OBJECTIVE="${RUN_SCOPE_OBJECTIVE:-Continue from r3d with full cumulative replay from r2/r3b/r3c/r3d and a bounded generalized recovery delta that cannot dominate softer bridge and paraphrase surfaces.}"
export RUN_SCOPE_HYPOTHESIS="${RUN_SCOPE_HYPOTHESIS:-A cumulative continuation from the r3d winner with replay kept dominant and corrective surfaces balance-capped should preserve the learned visible and hidden routing gains while shrinking the remaining family, form, and syntax miss classes more safely than r3e.}"
export RUN_SCOPE_PROMPT_CONTRACT="${RUN_SCOPE_PROMPT_CONTRACT:-Keep the frozen spec19 bounded-intent user contract and shared bundle output surface unchanged. Change only the cumulative train mixture by appending a small balanced recovery delta on top of the winner curriculum.}"
