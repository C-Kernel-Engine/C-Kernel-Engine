#!/usr/bin/env python3
"""Preflight gates for spec08 rich scene-DSL training."""
...
from build_spec08_probe_contract_v7 import build_contract as build_probe_contract
...
from render_svg_structured_scene_rich_v7 import _parse_scene_document, render_structured_scene_rich_svg
...
def _validate_scene_output(output_tokens: str, expected_svg: str | None, prompt: str) -> dict[str, Any]:
    ...
    try:
        rendered_svg = render_structured_scene_rich_svg(output_tokens)
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec08 render catalog must be a JSON list")
    ...
    return {"count": int(total), "pass_count": int(ok), "pass_rate": float(ok) / float(max(1, total)), "layout_summary": per_layout, "failures": failures, "pass": int(ok) == int(total)}
...
def _canary_validation(run_dir: Path, prefix: str, per_split: int) -> dict[str, Any]:
    ...
    return {"count": int(total), "pass_count": int(ok), "pass_rate": float(ok) / float(max(1, total)), "cases": cases, "failures": failures, "pass": int(ok) == int(total)}
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Compute spec08 training budgets and run rich scene-DSL preflight gates")
    ap.add_argument("--run", required=True, type=Path, help="Run directory containing the staged dataset")
    ap.add_argument("--prefix", default="spec08_rich_scene_dsl", help="Dataset prefix")
    ...
    print("[spec08-preflight] " f"catalog pass={report['catalog']['pass']} rows={report['catalog']['count']} pass_rate={report['catalog']['pass_rate']:.4f}")
    print("[spec08-preflight] " f"canary pass={report['canary']['pass']} cases={report['canary']['count']} pass_rate={report['canary']['pass_rate']:.4f}")

    if args.strict and not passed:
        raise SystemExit("spec08 preflight failed: " + ", ".join(reasons))
    return 0
