...
class NumericalExecutionContractTests(unittest.TestCase):
    ...
    def test_kernel_interface_migration_debt_does_not_regress(self):
        ...
        self.assertEqual(report["counts"]["kernel_maps"], 369)
        self.assertEqual(report["counts"]["physical_layout_maps"], 6)
        self.assertEqual(report["counts"]["resolver_governed_maps"], 141)
        self.assertEqual(report["counts"]["interface_hardened_maps"], 91)
        self.assertEqual(
            report["counts"]["interface_abi_crossvalidated_maps"], 91
        )
        self.assertEqual(report["counts"]["contract_pending_maps"], 50)
        self.assertEqual(report["counts"]["map_owned_call_abi"], 242)
        self.assertEqual(report["counts"]["legacy_interface_ready_maps"], 68)
        self.assertEqual(report["counts"]["selection_managed_maps"], 98)
