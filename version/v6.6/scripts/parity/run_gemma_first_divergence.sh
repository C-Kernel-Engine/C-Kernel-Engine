...
DEFAULT_LOCAL_GGUF="$HOME/.cache/ck-engine-v6.6/models/unsloth--gemma-3-270m-it-GGUF/gemma-3-270m-it-Q5_K_M.gguf"
if [[ $# -ge 1 && -n "${1}" ]]; then
  MODEL_URI="$1"
else
...
V66_SCRIPTS_DIR="${ROOT}/version/v6.6/scripts"
PARITY_DIR="${V66_SCRIPTS_DIR}/parity"

# Infer work dir following ck_run_v6_6.py cache behavior.
WORK_DIR=""
if [[ "$MODEL_URI" == hf://* ]]; then
  repo_path="${MODEL_URI#hf://}"
  repo_path="${repo_path%/*}" # drop filename
  repo_key="${repo_path//\//--}"
  WORK_DIR="$HOME/.cache/ck-engine-v6.6/models/${repo_key}"
elif [[ "$MODEL_URI" == *.gguf ]]; then
  stem="$(basename "${MODEL_URI}" .gguf)"
  WORK_DIR="$HOME/.cache/ck-engine-v6.6/models/${stem}"
fi
if [[ -z "$WORK_DIR" ]]; then
  WORK_DIR="$HOME/.cache/ck-engine-v6.6/models/gemma-3-270m-it-Q5_K_M"
fi
...
echo "[2/4] Run ck_run_v6_6.py (broad llama+CK dumps)"
python "${V66_SCRIPTS_DIR}/ck_run_v6_6.py" run \
  "${MODEL_URI}" \
...
if [[ -f "${REF_DUMP}" ]]; then
  python "${V66_SCRIPTS_DIR}/parity_test.py" \
    --ck-dump "${CK_DUMP}" \
    --ref-dump "${REF_DUMP}"
else
