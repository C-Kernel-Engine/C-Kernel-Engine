#!/usr/bin/env python3
"""Preflight gates for spec15a memory-map scene-DSL training."""
...
from build_spec15a_probe_contract_v7 import build_contract as build_probe_contract
...
def _validate_scene_output(output_tokens: str, expected_svg: str | None, prompt: str, content_json: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        rendered_svg = render_structured_scene_spec15a_svg(output_tokens, content=content_json)
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec15a render catalog must be a JSON list")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Preflight gates for spec15a memory-map scene-DSL training")
    ...
    payload = {
        "schema": "ck.spec15a_preflight.v1",
        "run": str(run_dir),
        "prefix": prefix,
        "seq_len": int(args.seq_len),
