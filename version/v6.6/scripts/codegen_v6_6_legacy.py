...
- Kernel tables are loaded from KERNEL_REGISTRY.json (data-driven)
...
# ---------------------------------------------------------------------------
# KERNEL TABLE LOADING FROM REGISTRY (Data-Driven)
# ---------------------------------------------------------------------------
# These tables are built from KERNEL_REGISTRY.json at module load time.
# Adding a new kernel map file automatically makes it available.
# ---------------------------------------------------------------------------

def _load_kernel_registry() -> Dict:
    """Load kernel registry from kernel_maps/KERNEL_REGISTRY.json."""
    registry_path = _SCRIPT_DIR.parent / "kernel_maps" / "KERNEL_REGISTRY.json"
    if not registry_path.exists():
        print(f"[codegen] Warning: KERNEL_REGISTRY.json not found at {registry_path}")
        return {"kernels": []}
    with registry_path.open("r", encoding="utf-8") as f:
        return json.load(f)


def _build_kernel_tables(registry: Dict) -> Tuple[Dict, Dict, Dict, Dict]:
    """Build kernel tables from registry.

    Returns:
        (DTYPE_TO_GEMM_NT_KERNEL, DTYPE_TO_EMBEDDING_KERNEL,
         DTYPE_TO_GEMV_Q8_KERNEL, DTYPE_TO_GEMM_NT_Q8_KERNEL)

    Note: For FP32 activation fallback, we derive kernel names from the
    quantized-activation kernels. E.g., gemm_nt_q5_0_q8_0 -> gemm_nt_q5_0
    """
    gemm_nt = {"fp32": "gemm_blocked_serial"}
    embedding = {"fp32": "embedding_forward"}
    gemv_q8 = {}
    gemm_nt_q8 = {}
    ...
    for k in kernels:
        kernel_op = k.get("op", "")
        kernel_id = k.get("id", k.get("name", ""))
        impl = k.get("impl", {})
        kernel_fn = impl.get("function", kernel_id)
        quant = k.get("quant", {})
        weight_dtype = quant.get("weight", "").lower()
        act_dtype = quant.get("activation", "").lower()

        # Skip kernels with mixed weight types (contain |)
        if "|" in weight_dtype:
            continue

        # GEMM NT kernels (prefill)
        if kernel_op == "gemm" and kernel_id.startswith("gemm_nt_"):
            if act_dtype in ("q8_0", "q8_k"):
                # INT8 activation path: (kernel, act_type, quantize_fn)
                quantize_fn = f"quantize_row_{act_dtype}"
                if weight_dtype:
                    gemm_nt_q8[weight_dtype] = (kernel_fn, act_dtype, quantize_fn)
                    # Also derive FP32 activation fallback: gemm_nt_q5_0_q8_0 -> gemm_nt_q5_0
                    # (This fallback kernel dequantizes weights to FP32 internally)
                    if weight_dtype not in gemm_nt:
                        gemm_nt[weight_dtype] = f"gemm_nt_{weight_dtype}"
            elif act_dtype == "fp32" or not act_dtype:
                # FP32 activation path
                if weight_dtype:
                    gemm_nt[weight_dtype] = kernel_fn

        # GEMV kernels (decode)
        elif kernel_op == "gemv" and kernel_id.startswith("gemv_"):
            if act_dtype in ("q8_0", "q8_k"):
                # INT8 activation path: (kernel, act_type, quantize_fn)
                quantize_fn = f"quantize_row_{act_dtype}"
                if weight_dtype:
                    gemv_q8[weight_dtype] = (kernel_fn, act_dtype, quantize_fn)

        # Embedding kernels
        elif kernel_op == "embedding":
            if weight_dtype:
                embedding[weight_dtype] = kernel_fn

    return gemm_nt, embedding, gemv_q8, gemm_nt_q8


# Load registry and build tables at module import
_KERNEL_REGISTRY = _load_kernel_registry()
DTYPE_TO_GEMM_NT_KERNEL, DTYPE_TO_EMBEDDING_KERNEL, \
    DTYPE_TO_GEMV_Q8_KERNEL, DTYPE_TO_GEMM_NT_Q8_KERNEL = _build_kernel_tables(_KERNEL_REGISTRY)
...
#   2. Add kernel map JSON file in version/v6.6/kernel_maps/
#   3. Run gen_kernel_registry_from_maps.py (or ck_run_v6_6.py will auto-regen)
#   4. Implement the kernel in src/kernels/
#   5. Codegen will automatically pick up the kernel from the registry
# =============================================================================
...
def emit_c_source_v6(layout: v3.ModelLayout,
    ...
    add()
    add("#ifdef __linux__")
    add("    model->weights_base = mmap(NULL, total,")
    add("                       PROT_READ | PROT_WRITE,")
    add("                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB,")
    add("                       -1, 0);")
    add("    if (model->weights_base == MAP_FAILED) {")
    add("        model->weights_base = mmap(NULL, total,")
    add("                           PROT_READ | PROT_WRITE,")
    ...
    add("    }")
    add("#else")
    add("    model->weights_base = aligned_alloc(64, total);")
    add("    if (!model->weights_base) {")
    ...
    add("    MagicHeader *header = (MagicHeader *)model->weights_base;")
    ...
    add(f"        uint32_t *ptr = (uint32_t*)((char*)model->weights_base + {safe_name}_CANARIES[i].offset);")
    ...
    add("    if (!model || !model->weights_base) return;")
    add("#ifdef __linux__")
    add("    munmap(model->weights_base, model->total_bytes);")
    add("#else")
    add("    free(model->weights_base);")
    add("#endif")
    add("    model->weights_base = NULL;")
    ...
    add()
    add(f"    for (int i = 0; i < {safe_name}_CANARY_COUNT; i++) {{")
    add(f"        ptr = (uint32_t*)((char*)model->weights_base + {safe_name}_CANARIES[i].offset);")
