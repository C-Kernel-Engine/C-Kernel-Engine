...
RUN="${1:-$MODEL_CACHE_ROOT/train/spec19_scene_bundle_l3_d192_h384_ctx768_r3d_sft_b_instruction}"
...
run_timed materialize_dataset \
  python3 "$MATERIALIZE_SCRIPT" \
    ...
    --line-name "spec19_sft_instruction_b" \
    --format-version "ck.spec19_sft_instruction_b.v1" \
    --train-surface-multiplier "clean_stop_anchor=3" \
    --train-surface-multiplier "explicit_bundle_anchor=3" \
    --train-surface-multiplier "explicit_permuted_anchor=3" \
...
run_timed run_scope_init \
  python3 version/v7/scripts/init_run_scope_v7.py \
    ...
    --title "Spec19 R3d SFT-B Anchor Floor Follow-Up" \
    --objective "Start from the frozen r3d winner and run a lower-overwrite instruction SFT pass that keeps the unified routing curriculum while restoring explicit-anchor and clean-stop support." \
    --hypothesis "A tighter-budget SFT pass with explicit anchor floors should preserve the held-out routing gains from the first SFT run while avoiding the syntax and anchor regressions caused by over-weighted routing surfaces." \
    --prompt-contract "Keep the frozen spec19 bounded-intent user prompt contract unchanged. Instruction-style rows may restate that same request, but the external probe contract stays the same." \
    ...
    --success-gate "Match or beat r3d on aggregate exactness while keeping visible held-out fidelity at 10/10 dev and test." \
    --success-gate "Reduce train explicit-anchor syntax regressions relative to the first SFT run." \
    ...
    --lesson-learned "The first SFT run improved visible held-out fidelity but regressed train explicit-anchor stability and syntax." \
    --lesson-learned "Anchor and clean-stop surfaces were underweighted relative to routebook and bridge instruction rows." \
    --lesson-learned "For this small model, alignment should bias toward deterministic prompt-following without overwhelming the base policy manifold." \
...
printf 'spec19 sft_b finished for %s\n' "$RUN"
