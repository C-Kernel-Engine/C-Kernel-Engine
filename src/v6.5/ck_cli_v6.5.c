/*
 * C-Kernel-Engine v6.5 Native CLI
 *
 * Features:
 ...
 *
 * Usage:
 *   ck-cli-v6.5 --model <name>                    # Auto-discover from cache
 *   ck-cli-v6.5 <libmodel.so> <weights.bump>      # Direct paths
 *   ck-cli-v6.5 --lib <.so> --weights <.bump>     # Named args
 */
...
static const char *get_cache_dir(void) {
    ...
    if (!home) home = "/tmp";
    snprintf(cache_path, sizeof(cache_path), "%s/.cache/ck-engine-v6.5/models", home);
    return cache_path;
}
