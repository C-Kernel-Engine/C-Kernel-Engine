#!/usr/bin/env python3
"""
Check layer-0 post-attention chain contracts:
  1) post_attention_norm output vs local RMSNorm reference of out_proj output
  2) residual_add output vs (post_attention_norm_output + saved residual)

TODO(model-contracts):
  - Keep as a per-family decode-path contract test (not Gemma-only).
  - Validate eps/norm variant from model contract metadata when available.
"""
...
def main() -> int:
    ...
    if None in (res0_op, out_op, pan_op, add_op):
        raise RuntimeError("missing one of required ops: residual_save(0), out_proj, post_attention_norm, residual_add(0)")
    ...
    try:
        ...
        y_pan = read_f32(base_ptr, pan_abs, embed_dim)
