...
| Qwen3.5 35B-A3B | `hf://unsloth/Qwen3.5-35B-A3B-GGUF/Qwen3.5-35B-A3B-Q4_K_M.gguf` | High-memory artifact; follow the v8 runbook's Qwen long-context lane |
| Qwen3.6 27B | `hf://ggml-org/Qwen3.6-27B-GGUF/Qwen3.6-27B-Q4_K_M.gguf` | High-memory artifact; optimized prefill and its parity boundary are documented in the v8 runbook |
| Qwen3.8 27B dense | `hf://ggml-org/Qwen3.8-27B-GGUF/Qwen3.8-27B-Q4_K_M.gguf` | High-memory artifact; use a fresh run directory with `--force-convert --force-compile` and follow the v8 runbook's tested command and evidence boundary |
...
| Cohere2 Command R | `hf://bartowski/c4ai-command-r7b-12-2024-GGUF/c4ai-command-r7b-12-2024-Q4_K_M.gguf` | `--chat-template auto --temperature 0.0` |
| Cohere North Mini Code | `hf://ji-farthing/North-Mini-Code-1.0-ik-llama-validation-GGUF/North-Mini-Code-1.0-ik_llama-Q4_K_M.gguf` | `--chat-template auto --temperature 0.0`; high-memory MoE lane |
...
| Cohere Transcribe 2B | [`hf://cstr/cohere-transcribe-03-2026-GGUF/cohere-transcribe.gguf`](https://huggingface.co/cstr/cohere-transcribe-03-2026-GGUF) | Mixed FP16/FP32 BUMP native-kernel path: exact short trajectory, deterministic five-minute oracle match, and complete 42-minute bounded execution; use the [Cohere kernels and Transcribe guide](https://c-kernel-engine.github.io/C-Kernel-Engine/v8-cohere-kernels.html) |
...
copy-paste table: the public Unsloth release is a four-shard UD recipe, while
CKE's current evidence belongs to a different 119,150,722,112-byte local GGUF.
The public artifact must be tested independently before it is advertised as a
directly runnable substitute.
...
|---|---|---:|---|
| Qwen2 0.5B, Qwen3 0.6B, Llama/Nanbeige 3B, GPT-2 | Coherent GGUF end-to-end nightly lanes | 1,024 tokens where recorded | Commodity x86 nightly lanes; provider selected from detected ISA |
| Gemma3 270M IT Q4_K_M and UD-Q4_K_XL | 256/256 complete-vocabulary rows bit-exact to llama.cpp for both artifacts via a circuit-declared numerical schedule | 520-token boundary nearly exact but differs at five rows; longer-context bit-exact certification open | Commodity x86 nightly lanes; schedule contract documented in the numerical-contracts case study |
...
| Qwen3.8 Flash Next 120B-A6B Q4_K_M | 40-token batched prompt plus 64 full-vocabulary decode comparisons bit-exact to llama.cpp including the prefill row; all-layer X-Ray reported 45,285 exact comparisons with zero value differences | 8,192-token configured capacity (capacity, not consumed-context certification); BF16, long-context, and performance certification open | Ryzen 9 9950X3D Zen 5 artifact runs |
...
| Cohere Transcribe 2B mixed FP16/FP32 | Exact 32-token short trajectory; two five-minute runs reproduce every token/timestamp and match the 686-word CrispASR transcript with zero edits; complete 42-minute VAD-sliced run differs from CrispASR by one word out of 6,889 | English audio with externally supplied Silero VAD slices up to 30 seconds; native VAD, multilingual, generated-circuit, concurrent-session, and quantized execution remain open | Ryzen 9 9950X3D; five-minute RTF 0.220–0.225 and about 8.51 GB peak RSS; 42-minute RTF 0.252 and 8.75 GB peak RSS |
...
| Cohere Transcribe 2B | Native bounded long-audio E2E | The mixed FP16/FP32 BUMP path accounts for all 2,104 tensors and runs the 48-layer encoder plus 8-layer cross-attention decoder through CKE kernels. It matches the short 32-token trajectory exactly, matches a 686-word five-minute CrispASR transcript with zero edits across deterministic repeats, accepts PCM16 input through the CKE resampler, and completes the retained 42-minute recording through bounded VAD speech slices. Native VAD, independent timestamp accuracy, multilingual, generated-circuit, concurrent-session, quantized, and optimized execution remain open. |
...
Human-facing documentation belongs in the HTML documentation site, not new hidden Markdown reports under `docs/logs`, `docs/notes`, or the repository root. Update the relevant `docs/site/*.html` page with the explanation, measured evidence, limitations, and navigation links. Markdown is useful for temporary agent-to-agent handoffs outside tracked documentation; it is not a substitute for publishing the result. Repository entry points, legal/community documents, and the PR template are explicit exceptions. Existing Markdown is migration debt, not a precedent for adding more. See the [documentation policy and cleanup plan](https://c-kernel-engine.github.io/C-Kernel-Engine/contributing.html#documentation-policy).
