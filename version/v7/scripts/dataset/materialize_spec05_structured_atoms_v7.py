#!/usr/bin/env python3
"""Materialize a cache-local spec05 workspace from the structured SVG line."""
...
STRUCTURED_TOY_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec05_v7.py"
...
SHAPE_CANDIDATES = ("circle", "rect", "triangle")
COLOR_CANDIDATES = ("blue", "green", "orange", "purple", "red", "teal", "gold", "gray")
LABEL_CANDIDATES = ("map", "plan", "flow", "go", "sync", "ok", "ai", "data", "note", "ops")
...
def _prompt_from_tags(tags: dict[str, str], *, include_edit: str | None = None) -> str:
    ...
    if "frame" in tags:
        tokens.append(f"[frame:{tags.get('frame', 'plain')}]")
    if "density" in tags:
        tokens.append(f"[density:{tags.get('density', 'compact')}]")
    if include_edit:
...
def _sft_instruction_variants(prompt: str) -> list[str]:
    ...
    frame = _frame_phrase(tags.get("frame", "plain"))
    density = _density_phrase(tags.get("density", "compact"))

    variants: list[str]
    if layout == "single":
        variants = [
            f"Draw a {size} {color} {shape} centered on {bg}, {frame}, with {density}. Return structured SVG atoms only. [OUT]",
            f"Create one {color} {shape} on {bg}. Keep it {size}, use {density}, {frame}, and output only structured SVG atom tags. [OUT]",
        ]
    elif layout == "pair-h":
        variants = [
            f"Draw two {size} shapes side by side on {bg}, {frame}, with {density}: a {color} {shape} on the left and a {color2} {shape2} on the right. Return structured SVG atoms only. [OUT]",
            f"Create a horizontal composition on {bg} with a left {color} {shape} and a right {color2} {shape2}. Use {density}, {frame}, and output structured SVG atom tags only. [OUT]",
        ]
    elif layout == "pair-v":
        variants = [
            f"Draw two {size} shapes stacked vertically on {bg}, {frame}, with {density}: a {color} {shape} on top and a {color2} {shape2} below. Return structured SVG atoms only. [OUT]",
            f"Create a vertical composition on {bg} with a top {color} {shape} and a bottom {color2} {shape2}. Use {density}, {frame}, and output structured SVG atom tags only. [OUT]",
        ]
    elif layout == "label-card":
        variants = [
            f"Create a rounded {color} label card on {bg} with the text {label} centered, {frame}, with {density}. Return structured SVG atoms only. [OUT]",
            f"Make a text card on {bg}: a {color} rectangle with {label} in the middle. Use {density}, {frame}, and output structured SVG atom tags only. [OUT]",
        ]
    elif layout == "badge":
        variants = [
            f"Create a badge card on {bg} with a {color} {shape} icon and the text {label}, {frame}, with {density}. Return structured SVG atoms only. [OUT]",
            f"Draw a badge on {bg}: a card with a {color} {shape} icon on the left and {label} on the right. Use {density}, {frame}, and output structured SVG atom tags only. [OUT]",
        ]
    else:
        variants = [f"{prompt}"]
...
def _flip_bg(bg: str) -> str | None:
    current = str(bg or "").strip()
    for value in ("paper", "mint", "slate", "none"):
        if value != current:
            return value
    return None
...
def _alt_values(current: str, candidates: tuple[str, ...], *, limit: int | None = None) -> list[str]:
...
def _flip_frame(frame: str) -> str | None:
    return {
        "plain": "card",
        "card": "plain",
    }.get(str(frame or "").strip())


def _paired_shape(shape: str, *, vertical: bool) -> str:
    mapping = {
        "circle": "triangle" if vertical else "rect",
        "rect": "circle" if vertical else "triangle",
        "triangle": "rect" if vertical else "circle",
    }
    return mapping.get(shape, "rect")


def _paired_color(color: str, *, vertical: bool) -> str:
    ordered = ("red", "blue", "green", "orange", "purple", "teal", "gold", "gray")
    current = str(color or "").strip()
    if current not in ordered:
        return ordered[0]
    idx = ordered.index(current)
    offset = 3 if vertical else 1
    return ordered[(idx + offset) % len(ordered)]


def _alt_label(label: str) -> str | None:
    return _alt_value(str(label or ""), LABEL_CANDIDATES)


def _label_edit_targets(label: str, *, limit: int) -> list[str]:
    return _alt_values(str(label or ""), LABEL_CANDIDATES, limit=limit)


def _label_to_shape(label: str) -> str:
    mapping = {
        "ai": "circle",
        "data": "rect",
        "flow": "triangle",
    ...
    return mapping.get(str(label or "").strip(), "rect")


def _label_from_tags(tags: dict[str, str]) -> str:
    if str(tags.get("label") or "").strip():
        return str(tags.get("label") or "").strip()
    shape = str(tags.get("shape") or "rect").strip()
    color = str(tags.get("color") or "blue").strip()
    mapping = {
        ("circle", "red"): "go",
        ("circle", "teal"): "sync",
        ("rect", "green"): "data",
        ("rect", "gray"): "ops",
        ("triangle", "orange"): "flow",
        ("triangle", "gold"): "map",
    }
    return mapping.get((shape, color), "note")
...
def _candidate_midtrain_edits(tags: dict[str, str]) -> list[tuple[str, dict[str, str]]]:
    ...
    if layout == "single":
        shape_alt = _alt_value(str(tags.get("shape") or ""), SHAPE_CANDIDATES)
        if shape_alt:
            target = dict(tags)
            target["shape"] = shape_alt
            edits.append((f"shape={shape_alt}", target))
        color_alt = _alt_value(str(tags.get("color") or ""), COLOR_CANDIDATES)
        if color_alt:
            target = dict(tags)
        ...
        pair_target = dict(tags)
        pair_target["layout"] = "pair-h"
        pair_target["shape2"] = _paired_shape(str(tags.get("shape") or "rect"), vertical=False)
        pair_target["color2"] = _paired_color(str(tags.get("color") or "blue"), vertical=False)
        edits.append(("compose=pair-h", pair_target))
        badge_target = dict(tags)
        badge_target["layout"] = "badge"
        badge_target["label"] = _label_from_tags(tags)
        edits.append(("compose=badge", badge_target))
    elif layout == "pair-h":
        target = dict(tags)
    ...
    if layout in {"pair-h", "pair-v"}:
        shape2_alt = _alt_value(str(tags.get("shape2") or ""), SHAPE_CANDIDATES)
        if shape2_alt and shape2_alt != tags.get("shape"):
            target = dict(tags)
            target["shape2"] = shape2_alt
            edits.append((f"shape2={shape2_alt}", target))
        color2_alt = _alt_value(str(tags.get("color2") or ""), COLOR_CANDIDATES)
        ...
        if size_alt:
            ...
            target["size"] = size_alt
            edits.append((f"size={size_alt}", target))
        single_target = dict(tags)
        single_target["layout"] = "single"
        single_target.pop("shape2", None)
        single_target.pop("color2", None)
        edits.append(("simplify=single", single_target))
        badge_target = dict(tags)
        badge_target["layout"] = "badge"
        badge_target["label"] = _label_from_tags(tags)
        badge_target.pop("shape2", None)
        badge_target.pop("color2", None)
        edits.append(("compose=badge", badge_target))
    elif layout == "label-card":
        for label_alt in _label_edit_targets(str(tags.get("label") or ""), limit=2):
            target = dict(tags)
        ...
        badge_target = dict(tags)
        badge_target["layout"] = "badge"
        badge_target["shape"] = _label_to_shape(str(tags.get("label") or ""))
        edits.append(("compose=badge", badge_target))
        single_target = dict(tags)
        single_target["layout"] = "single"
        single_target["shape"] = _label_to_shape(str(tags.get("label") or ""))
        single_target["size"] = "small"
        single_target.pop("label", None)
        edits.append(("simplify=single", single_target))
    elif layout == "badge":
        for label_alt in _label_edit_targets(str(tags.get("label") or ""), limit=2):
            target = dict(tags)
        ...
        if color_alt:
            ...
            target["color"] = color_alt
            edits.append((f"color={color_alt}", target))
        label_card_target = dict(tags)
        label_card_target["layout"] = "label-card"
        label_card_target.pop("shape", None)
        edits.append(("compose=label-card", label_card_target))
        single_target = dict(tags)
        single_target["layout"] = "single"
        single_target["size"] = "small"
        single_target.pop("label", None)
...
def _midtrain_edit_priority(layout: str) -> tuple[str, ...]:
    if layout == "single":
        return ("compose", "density", "frame", "bg", "size", "color", "shape")
    if layout in {"pair-h", "pair-v"}:
        return ("color2", "shape2", "layout", "compose", "simplify", "density", "frame", "bg", "size")
    if layout == "label-card":
        return ("label", "compose", "simplify", "density", "frame", "bg", "color")
    if layout == "badge":
        return ("label", "compose", "simplify", "density", "frame", "bg", "shape", "color")
    return ()
...
def _boosted_edit_repeat(layout: str, edit_op: str, base_repeat: int) -> int:
    edit_kind = str(edit_op or "").split("=", 1)[0]
    repeat = max(1, int(base_repeat))
    if layout in {"pair-h", "pair-v"} and edit_kind == "color2":
        return max(repeat, 3)
    if layout in {"pair-h", "pair-v"} and edit_kind == "shape2":
        return max(repeat, 2)
    if layout == "label-card" and edit_kind == "label":
        return max(repeat, 2)
    if layout == "badge" and edit_kind == "label":
        return max(repeat, 2)
    return repeat
    ...
    *,
    ...
    for row in catalog_rows:
        ...
        for edit_op, target_tags in edits:
            ...
            repeat = _boosted_edit_repeat(layout, edit_op, int(edit_repeat)) if is_train else 1
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Blended base coverage plus interleaved same-layout and cross-layout edit rows across single, pair, label-card, and badge layouts.",
                "counts": {
            ...
            "sft": {
                ...
                "notes": "Layout-balanced mixed DSL plus natural-language instruction prompts that mention frame and density composition controls.",
                "counts": {
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec05 workspace from the structured SVG line")
    ...
    ap.add_argument("--prefix", default="spec05_structured_svg_atoms", help="Artifact prefix for generated files")
