...
RUN="${1:-$MODEL_CACHE_ROOT/train/spec_broader_1_scene_dsl_l3_d192_h384_ctx512_r1}"
DATASET_PREFIX="${DATASET_PREFIX:-spec_broader_1_scene_dsl}"
...
MIDTRAIN_DIRECT_REPEAT="${MIDTRAIN_DIRECT_REPEAT:-4}"
MIDTRAIN_CLOSE_REPEAT="${MIDTRAIN_CLOSE_REPEAT:-6}"
TRAIN_REPEATS="${TRAIN_REPEATS:-3}"
HOLDOUT_REPEATS="${HOLDOUT_REPEATS:-1}"
PROBE_PER_SPLIT="${PROBE_PER_SPLIT:-14}"
...
PROBE_CONTRACT="$RUN/spec_broader_1_probe_contract.json"
PREFLIGHT_JSON="$RUN/spec_broader_1_preflight.json"
...
python3 version/v7/scripts/check_disk_headroom_v7.py \
  --path "$RUN" \
  --min-free-gb 20 \
  --label "spec_broader_1_preflight"

python3 version/v7/scripts/dataset/materialize_spec_broader_1_scene_dsl_v7.py \
  --workspace "$WORKSPACE" \
...
if [ "$RUN_PREFLIGHT" = "1" ]; then
  PREFLIGHT_ARGS=(
    python3
    version/v7/scripts/spec_broader_1_preflight_v7.py
    --run "$RUN"
    --prefix "$DATASET_PREFIX"
...
printf '[spec-broader-1-budget] pretrain_total_tokens=%s midtrain_total_tokens=%s\n' "$PRETRAIN_TOTAL_TOKENS" "$MIDTRAIN_TOTAL_TOKENS"
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec_broader_1_scene_dsl_pretrain_midtrain_v7.sh",
  ...
  "stages": [
    {
      ...
      "enabled": true,
      "datasets": [
        {"name": "$(basename "$PRETRAIN_DATA")", "path": "$PRETRAIN_DATA", "kind": "generated_dataset"}
      ],
      "runs": []
    },
    {
      ...
      "enabled": true,
      "datasets": [
        {"name": "$(basename "$MIDTRAIN_DATA")", "path": "$MIDTRAIN_DATA", "kind": "generated_dataset"}
      ],
      "runs": []
    },
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 3e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec_broader_1_stage_a.json"
...
.venv/bin/python version/v7/scripts/train_data_pipeline_v7.py \
  ...
  --lr 2.5e-4 \
  --no-post-train-eval \
  --json-out "$RUN/train_spec_broader_1_stage_b.json"

if [ "$RUN_STAGE_EVAL" = "1" ]; then
  .venv/bin/python version/v7/scripts/eval_stage_v7.py --run "$RUN" --all-stages --n-samples 5
else
  echo "[stage-eval] skipped builtin default probes; use spec_broader_1_probe_report.json/html for asset-grounded scene evaluation."
fi

python3 version/v7/scripts/build_spec_broader_1_probe_contract_v7.py \
  --run "$RUN" \
  --prefix "$DATASET_PREFIX" \
...
python3 version/v7/scripts/build_probe_report_v7.py \
  ...
  --output "$RUN/spec_broader_1_probe_report.html" \
  --json-out "$RUN/spec_broader_1_probe_report.json"

python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
  --probe-report "$RUN/spec_broader_1_probe_report.json" \
  --output-html "$RUN/spec_broader_1_tested_prompts_report.html" \
  --output-md "$RUN/spec_broader_1_tested_prompts_report.md"
...
printf 'spec_broader_1 pretrain+midtrain finished for %s\n' "$RUN"
