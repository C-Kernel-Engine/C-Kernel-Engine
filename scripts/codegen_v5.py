#!/usr/bin/env python3
"""
codegen_v5.py - Explicit unrolled C source emitter for IR v4 layouts.
...
def emit_c_source_v5(layout: v3.ModelLayout,
    ...
    add(f" * @brief AUTO-GENERATED: {layout.name} Implementation (IR v5 - Explicit Unrolled)")
    add(f" *")
    ...
    add(f'#error "{layout.name}: v5 codegen currently supports fp32 only. Use --dtype=fp32."')
    add("#endif")
    add()
    ...
    if mode == "decode":
        add("/* ============================================================================")
        add(" * EXPLICIT PER-LAYER DECODE FUNCTIONS (v5 unrolled)")
        add(" * ============================================================================ */")
        add()
        ...
        for layer_id in range(num_layers):
            ...
            add("    ck_residual_add_token_major(residual1, mlp_out, output, 1, aligned_embed_dim);")
            if emit_debug:
        ...
        # For prefill, we generate simpler code since it's less critical for debugging
        add("/* ============================================================================")
        add(" * PREFILL (v5 - simplified, not fully unrolled)")
        add(" * ============================================================================ */")
        add()
        add("/* Note: Prefill mode uses a loop for brevity. Use decode mode for full unrolling. */")
        add(f"void {safe_name_lower}_forward(")
        add(f"    {safe_name}Model *model,")
        add("    const int *tokens,")
        add("    int num_tokens")
        ...
        add(f"    const int aligned_context_window = {aligned_expr(aligned_context, str(config.get('max_position_embeddings', 32768)))};")
        add()
        add("    /* TODO: Implement prefill with explicit kernels if needed */")
        add('    fprintf(stderr, "v5 prefill not yet implemented\\n");')
        add("}")
        add()
    ...
    print(f"[v5.c] Written: {output_path}")
    print(f"[v5.c] Mode: {mode}, Layers: {num_layers} (explicit unrolled)")


if __name__ == "__main__":
    # This module is imported by build_ir_v5.py
    print("codegen_v5.py - Use build_ir_v5.py to generate code")
