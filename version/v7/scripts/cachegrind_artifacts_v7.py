#!/usr/bin/env python3
"""
Generate cachegrind_summary.json for v7 IR visualizer.
"""
...
def parse_top_functions(text: str, top_k: int = 20) -> List[Dict[str, object]]:
    ...
    for line in text.splitlines():
        ...
        if "PROGRAM TOTALS" in line or stripped.startswith(("#", "-", "I", "Events")):
            continue
        # cg_annotate uses percentage suffixes and table markers (<, >).
        # Strip these first, then parse the first seven integer columns.
        cleaned = re.sub(r"\([^)]*\)", "", line)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate cachegrind_summary.json for v7")
