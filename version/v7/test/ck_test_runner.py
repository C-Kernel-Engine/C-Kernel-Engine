...
def _candidate_model_dirs() -> list[Path]:
    ...
    env_model = os.environ.get("CK_V7_TEST_MODEL")
    if env_model:
        out.append(Path(env_model).expanduser())

    cache_bases = [
        ROOT_DIR / ".ck_cache",
        Path.home() / ".cache" / "ck-engine-v7" / "models",
    ]
...
class TestRunner:
    ...
    def memory_validation(self) -> int:
        ...
        if not self._ir_path().exists():
            ...
            c_file = self.model_path / "model_v7.c"
            if c_file.exists():
    ...
    def trace_token(self, token: int) -> int:
        ...
        return self.run_script("v7_comprehensive_debug.py", [
            "--verbose",
            f"--token={token}"
        ])

    def find_divergence(self, token: int = 25) -> int:
        """Find divergence in v7."""
        print("\n" + "=" * 70)
        print(f"FIND DIVERGENCE (token={token})")
        print("=" * 70)
        print("\nThis requires a cached v7 model directory.")
...
Examples:
    ...
    if model_path is None:
        print("ERROR: No valid v7 model cache directory found")
        print("Please run the pipeline to generate model files first:")
        print(f"  cd {TEST_DIR.parents[2]}")
        print("  python version/v7/scripts/ck_run_v7.py run \"hf://Qwen/Qwen2-0.5B-Instruct-GGUF/qwen2-0_5b-instruct-q4_k_m.gguf\" --force-compile")
        print("Optional override:")
        print("  CK_V7_TEST_MODEL=/path/to/model_dir make v7-test-quick")
        return 1
