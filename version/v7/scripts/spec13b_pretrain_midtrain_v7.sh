...
RUN="${1:-$MODEL_CACHE_ROOT/train/spec13b_scene_dsl_l3_d192_h384_ctx768_r1}"
DATASET_PREFIX="${DATASET_PREFIX:-spec13b_scene_dsl}"
FAMILY_MODE="${FAMILY_MODE:-all}"
WORKSPACE="$RUN/dataset"

clear_stale_postrun_artifacts() {
  local run_dir="$1"
  rm -f \
    "$run_dir/spec13b_probe_contract.json" \
    "$run_dir/spec13b_probe_report.json" \
    "$run_dir/spec13b_probe_report.html" \
    "$run_dir/spec13b_tested_prompts_report.html" \
    "$run_dir/spec13b_tested_prompts_report.md" \
...
run_timed() {
  ...
  printf '[spec13b-timing] stage=%s event=start started_at=%s\n' "$label" "$(timestamp_utc)"
  "$@"
  rc=$?
  ended_epoch="$(date +%s)"
  printf '[spec13b-timing] stage=%s event=end ended_at=%s elapsed_sec=%s\n' \
...
build_probe_artifacts() {
  ...
  python3 version/v7/scripts/build_spec13b_probe_contract_v7.py \
    --run "$run_dir" \
  ...
  python3 version/v7/scripts/build_probe_report_v7.py \
    ...
    --output "$run_dir/spec13b_probe_report.html" \
    --json-out "$run_dir/spec13b_probe_report.json"

  python3 version/v7/scripts/build_structured_scene_tested_prompts_doc_v7.py \
    --probe-report "$run_dir/spec13b_probe_report.json" \
    --output-html "$run_dir/spec13b_tested_prompts_report.html" \
    --output-md "$run_dir/spec13b_tested_prompts_report.md"
...
PRETRAIN_CANONICAL_REPEAT="${PRETRAIN_CANONICAL_REPEAT:-32}"
PRETRAIN_BRIDGE_REPEAT="${PRETRAIN_BRIDGE_REPEAT:-12}"
MIDTRAIN_CANONICAL_REPEAT="${MIDTRAIN_CANONICAL_REPEAT:-24}"
MIDTRAIN_BRIDGE_REPEAT="${MIDTRAIN_BRIDGE_REPEAT:-12}"
PRETRAIN_DECISION_BONUS_REPEAT="${PRETRAIN_DECISION_BONUS_REPEAT:-0}"
MIDTRAIN_DECISION_BONUS_REPEAT="${MIDTRAIN_DECISION_BONUS_REPEAT:-0}"

PROBE_PER_SPLIT="${PROBE_PER_SPLIT:-12}"
HIDDEN_PROBE_PER_SPLIT="${HIDDEN_PROBE_PER_SPLIT:-8}"
...
PROBE_CONTRACT="$RUN/spec13b_probe_contract.json"
PREFLIGHT_JSON="$RUN/spec13b_preflight.json"
...
MATERIALIZE_ARGS=(
  python3
  version/v7/scripts/dataset/materialize_spec13b_scene_dsl_v7.py
  --workspace "$WORKSPACE"
  --prefix "$DATASET_PREFIX"
  ...
  --pretrain-decision-bonus-repeat "$PRETRAIN_DECISION_BONUS_REPEAT"
  --midtrain-decision-bonus-repeat "$MIDTRAIN_DECISION_BONUS_REPEAT"
  --force
)
...
if [ "$RUN_PREFLIGHT" = "1" ]; then
  PREFLIGHT_ARGS=(
    python3
    version/v7/scripts/spec13b_preflight_v7.py
    --run "$RUN"
    --prefix "$DATASET_PREFIX"
...
printf '[spec13b-budget] pretrain_total_tokens=%s midtrain_total_tokens=%s\n' "$PRETRAIN_TOTAL_TOKENS" "$MIDTRAIN_TOTAL_TOKENS"
printf '[spec13b-telemetry] train_driver=%s ck_cli_log_every=%s analysis_checkpoints=%s train_save_every=%s train_save_final=%s\n' \
  "$TRAIN_DRIVER" "$CK_CLI_LOG_EVERY" "$ANALYSIS_CHECKPOINTS" "$TRAIN_SAVE_EVERY" "$TRAIN_SAVE_FINAL"
printf '[spec13b-policy] family_mode=%s dataset_prefix=%s\n' "$FAMILY_MODE" "$DATASET_PREFIX"
printf '[spec13b-policy] oracle=pytorch parity_regimen=python ck_runtime_bridge=ck_run production_train=ck_cli\n'
printf '[spec13b-policy] stage_a_mix=canonical:%s bridge:%s stage_b_mix=canonical:%s bridge:%s\n' \
  "$PRETRAIN_CANONICAL_REPEAT" "$PRETRAIN_BRIDGE_REPEAT" "$MIDTRAIN_CANONICAL_REPEAT" "$MIDTRAIN_BRIDGE_REPEAT"
printf '[spec13b-policy] decision_anchor_bonus=stage_a:%s stage_b:%s\n' \
  "$PRETRAIN_DECISION_BONUS_REPEAT" "$MIDTRAIN_DECISION_BONUS_REPEAT"
...
cat > "$RUN/training_plan.json" <<JSON
{
  ...
  "created_by": "spec13b_pretrain_midtrain_v7.sh",
...
PRETRAIN_CMD=(
  ...
  --lr 3e-4
  --no-post-train-eval
  --json-out "$RUN/train_spec13b_stage_a.json"
)
...
MIDTRAIN_CMD=(
  ...
  --lr 2.5e-4
  --no-post-train-eval
  --json-out "$RUN/train_spec13b_stage_b.json"
)
if [ "${#TRAIN_SAVE_FINAL_FLAG[@]}" -gt 0 ]; then
...
if [ "$RUN_STAGE_EVAL" = "1" ]; then
  ...
  echo "[stage-eval] skipped builtin default probes; use spec13b_probe_report.json/html for graph-family evaluation."
fi

build_probe_artifacts "$RUN" "$PROBE_CONTRACT" "$DATASET_PREFIX" "$CONTEXT_LEN" "$PROBE_PER_SPLIT" "$HIDDEN_PROBE_PER_SPLIT"
printf 'spec13b training finished for %s\n' "$RUN"
