#!/usr/bin/env python3
"""Materialize a cache-local spec19 textbook-routing bundle workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec19_v7.py"
BLUEPRINT_PATH = ROOT / "version" / "v7" / "reports" / "spec19_curriculum_blueprint.json"
LINE_NAME = "spec19_scene_bundle"
...
def _stage_surface_weights(stage: str) -> dict[str, int]:
    blueprint = _load_blueprint()
    blueprint_stage_id = STAGE_BLUEPRINT_IDS[str(stage or "").strip().lower()]
    for row in (blueprint.get("curriculum_stages") or []):
        if not isinstance(row, dict):
        ...
        for item in (row.get("surface_mix") or []):
            ...
            surface = str(item.get("surface") or "").strip()
            weight = int(item.get("weight") or 0)
            if surface and weight > 0:
                weights[surface] = weight
        if weights:
            return weights
    raise KeyError(f"missing stage surface weights for {stage}")
...
def materialize_workspace(
    ...
    *,
    ...
    base.shutil.copy2(BLUEPRINT_PATH, contracts_dir / "spec19_curriculum_blueprint.json")
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec19 keeps the frozen spec16 bundle contract and changes the prompt side to a textbook routebook plus minimal-pair mixture before widening style and topology."
            }
        }
    }
    ...
    mixture_manifest = {
        ...
        "stages": {
            "pretrain": {
                "blueprint_weights": _stage_surface_weights("pretrain"),
                "materialized_prompt_surface_counts": workspace_manifest["stages"]["pretrain"]["summary"]["prompt_surface_counts"],
                "train_rows": len(pretrain_train_rows),
            },
            "midtrain": {
                "blueprint_weights": _stage_surface_weights("midtrain"),
                "materialized_prompt_surface_counts": workspace_manifest["stages"]["midtrain"]["summary"]["prompt_surface_counts"],
                "train_rows": len(midtrain_train_rows),
            },
        },
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec19 scene-bundle workspace")
