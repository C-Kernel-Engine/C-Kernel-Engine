...
Generate a parent "Run Hub" HTML page that indexes v8 run directories under
~/.cache/ck-engine-v8/models (or a custom root). The hub links to each run's
ir_report.html and bridge artifacts so operators can jump between decoder-only
and encoder+decoder runs quickly.

Usage:
  python3 version/v8/tools/open_ir_hub_v8.py
  python3 version/v8/tools/open_ir_hub_v8.py --models-root ~/.cache/ck-engine-v8/models
  python3 version/v8/tools/open_ir_hub_v8.py --output /tmp/ir_hub_v8.html --index-out /tmp/runs_index_v8.json
...
│     - 📊 Open Dataset Viewer                                                 │
│     - 🩺 Weight Health Probe                                                 │
│   • 🔬 DSL Probe viewer — links to *probe_report.html for DSL output review  │
│   • Probe metrics in card stats (exact rate, probe count)                    │
...
│                                                                              │
│  TESTING:                                                                    │
│   python3 -m py_compile version/v8/tools/open_ir_hub_v8.py                  │
│   python3 version/v8/tools/open_ir_hub_v8.py --models-root ... --output ... │
│   python3 version/v8/tools/open_ir_visualizer_v8.py --generate --run ...    │
...
REPO_ROOT = V8_ROOT.parent.parent
KERNEL_REGISTRY_PATH = V8_ROOT / "kernel_maps" / "KERNEL_REGISTRY.json"
...
def _build_generate_report_cmd(run_dir: Path) -> str:
    ...
    return (
        "python3 version/v8/tools/open_ir_visualizer_v8.py "
        f"--generate --run {run_q} --html-only --strict-run-artifacts --output {out_q}"
    )


def _build_run_cmd(run_dir: Path, bridge_summary: dict[str, Any] | None = None) -> str:
    if isinstance(bridge_summary, dict) and bridge_summary.get("decoder_gguf"):
        cmd = [
            "version/v8/scripts/cks-v8-run",
            "run",
            str(bridge_summary["decoder_gguf"]),
        ]
        if bridge_summary.get("encoder_gguf"):
            cmd.extend(["--mmproj", str(bridge_summary["encoder_gguf"])])
        if bridge_summary.get("image_path"):
            cmd.extend(["--image-path", str(bridge_summary["image_path"])])
        if bridge_summary.get("prompt"):
            cmd.extend(["--prompt", str(bridge_summary["prompt"])])
        if bridge_summary.get("decoder_context_len"):
            cmd.extend(["--context-len", str(int(bridge_summary["decoder_context_len"]))])
        return " ".join(shlex.quote(part) for part in cmd)
    run_q = shlex.quote(str(run_dir))
    return f"version/v8/scripts/cks-v8-run run {run_q}"


def _build_bridge_parity_cmd(bridge_report_path: Path | None) -> str | None:
    if bridge_report_path is None:
        return None
    report_q = shlex.quote(str(bridge_report_path))
    return f"python3 version/v8/scripts/decoder_first_token_parity_v8.py --bridge-report {report_q}"


def _build_regression_fast_cmd() -> str:
    return "make regression-fast"


def _build_export_embeddings_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/tools/export_embeddings_v8.py {run_q}"


def _build_export_attention_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/tools/export_attention_v8.py {run_q} --probe"
...
def _build_prepare_all_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/tools/prepare_run_viewer_v8.py {run_q} --force"


def _build_model_make_cmd(target: str, run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"make {target} V8_MODEL={run_q}"


def _has_train_runtime(run_dir: Path) -> bool:
...
def _build_compile_train_cmd(run_dir: Path) -> str:
    return _build_run_make_cmd("v8-compile-train-runtime", run_dir)


def _build_compile_tokenizer_cmd() -> str:
    return "make tokenizer"


def _build_profile_dashboard_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"make v8-profile-dashboard-run RUN={run_q} V8_FORCE_COMPILE=1"


def _build_init_run_scope_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/scripts/init_run_scope_v8.py --run {run_q}"


def _find_report_path(run_dir: Path) -> Path | None:
    return _find_run_artifact(run_dir, "ir_report.html")


def _find_dataset_viewer_path(run_dir: Path) -> Path | None:
    return _find_run_artifact(run_dir, "dataset_viewer.html", "dataset/dataset_viewer.html")
...
def _artifact_search_bases(run_dir: Path) -> list[Path]:
    bases = [
        run_dir,
        run_dir / ".ck_build",
...
def _extract_bridge_summary(path: Path | None) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        return {}
    encoder_report = payload.get("encoder_report") if isinstance(payload.get("encoder_report"), dict) else {}
    decoder_runtime = payload.get("decoder_runtime") if isinstance(payload.get("decoder_runtime"), dict) else {}
    encoder_runtime = payload.get("encoder_runtime") if isinstance(payload.get("encoder_runtime"), dict) else {}
    source_size = encoder_report.get("source_image_size") if isinstance(encoder_report.get("source_image_size"), list) else []
    prompt = str(payload.get("prompt") or "").strip()
    generated = str(payload.get("generated_text") or payload.get("generated_text_raw") or "").strip()
    return {
        "bridge_mode": "encoder_decoder" if encoder_runtime else "decoder_only",
        "prompt": prompt,
        "prompt_excerpt": prompt[:220] if prompt else None,
        "generated_excerpt": generated[:220] if generated else None,
        "image_path": encoder_report.get("image_path"),
        "image_source": encoder_report.get("image_source"),
        "source_image_size": source_size if len(source_size) == 2 else None,
        "preprocess": encoder_report.get("preprocess"),
        "prefix_tokens": payload.get("prefix_tokens"),
        "prefix_grid_x": payload.get("prefix_grid_x"),
        "prefix_grid_y": payload.get("prefix_grid_y"),
        "prefix_text_pos": payload.get("prefix_text_pos"),
        "encoder_embed_dim": encoder_report.get("embed_dim"),
        "decoder_input_embed_dim": payload.get("decoder_input_embed_dim"),
        "decoder_context_len": payload.get("decoder_context_len"),
        "generated_token_count": payload.get("generated_token_count"),
        "decoder_gguf": decoder_runtime.get("gguf"),
        "encoder_gguf": encoder_runtime.get("gguf"),
    }
...
def _build_weight_health_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/scripts/weight_health_probe_v8.py --run {run_q}"


def _find_tokenizer_quality_gate_path(run_dir: Path) -> Path | None:
    candidate = run_dir / "tokenizer_quality_gate.json"
    if candidate.exists():
        return candidate
    return None


def _extract_tokenizer_quality_gate(run_dir: Path) -> dict[str, Any]:
    path = _find_tokenizer_quality_gate_path(run_dir)
    if path is None:
        return {}
    ...
    return {
        ...
        "content_mega_count": obj.get("content_mega_count", 0),
        "content_mega_fraction": obj.get("content_mega_fraction", 0),
        "issue_count": len(obj.get("issues", [])),
        "length_percentiles": obj.get("length_percentiles", {}),
    }


def _build_tokenizer_quality_gate_cmd(run_dir: Path) -> str:
    run_q = shlex.quote(str(run_dir))
    return f"python3 version/v8/scripts/tokenizer_quality_gate_v8.py --run {run_q}"
...
def _find_probe_report_html_path(run_dir: Path) -> Path | None:
    """Find the best probe report HTML viewer for this run."""
    candidates = sorted(run_dir.glob("*probe_report.html"))
    if candidates:
        return candidates[-1]
    return None


def _find_probe_contract_path(run_dir: Path) -> Path | None:
    """Find a probe contract suitable for build_probe_report_v8.py."""
    # Prefer a root-level contract (written by earlier pipeline steps).
    root_contracts = sorted(run_dir.glob("*probe_contract.json"))
    if root_contracts:
        return root_contracts[-1]
    # Fall back to dataset/contracts/ (materialized during dataset prep).
    ds_contracts = sorted(run_dir.glob("dataset/contracts/*probe_report_contract.json"))
    if ds_contracts:
        return ds_contracts[-1]
    return None


def _infer_line_slug(run_dir: Path) -> str:
    """Detect the LINE_SLUG for a run from its existing artifacts."""
    for pattern in ("*_preflight.json",):
        for path in sorted(run_dir.glob(pattern)):
            stem = path.stem                          # e.g. gen1_full_preflight
            suffix = "_preflight"
            if stem.endswith(suffix):
                return stem[: -len(suffix)]
    # Fall back: first segment of the run directory name before _scene_
    name = run_dir.name
    idx = name.find("_scene_")
    if idx > 0:
        return name[:idx]
    return "probe"


def _build_probe_report_cmd(run_dir: Path) -> str | None:
    """Build a command to generate DSL probe report for a run missing one."""
    # Already has a probe report → no need.
    if sorted(run_dir.glob("*probe_report.json")):
        return None
    contract = _find_probe_contract_path(run_dir)
    if contract is None:
        return None
    # Must have weights to actually run inference.
    if not (run_dir / "weights_manifest.json").exists() and not (run_dir / "weights.bump").exists():
        return None
    slug = _infer_line_slug(run_dir)
    run_q = shlex.quote(str(run_dir))
    contract_q = shlex.quote(str(contract))
    html_out = shlex.quote(str(run_dir / f"{slug}_probe_report.html"))
    json_out = shlex.quote(str(run_dir / f"{slug}_probe_report.json"))
    return (
        f"python3 version/v8/scripts/build_probe_report_v8.py "
        f"--run {run_q} --contract {contract_q} "
        f"--output {html_out} --json-out {json_out}"
    )
...
def _build_dsl_planner_cmd(run_dir: Path) -> str:
    ...
    return (
        "python3 version/v8/tools/open_dsl_planner_v8.py "
...
def _build_dataset_materialize_cmd(dataset_workspace: str | None, dataset_type: str | None) -> str | None:
    ...
    return (
        "python3 version/v8/scripts/materialize_svg_stage_artifacts_v8.py "
...
def _build_dataset_viewer_cmd(dataset_workspace: str | None, dataset_type: str | None, run_dir: Path) -> str:
    ...
    return (
        "python3 version/v8/scripts/build_svg_dataset_visualizer_v8.py "
...
class RunRecord:
    ...
    probe_report_html_path: Path | None
    probe_report_cmd: str | None
    ...
    run_cmd: str
    bridge_parity_cmd: str | None
    regression_fast_cmd: str
    export_embeddings_cmd: str
    export_attention_cmd: str
    prepare_all_cmd: str
    weight_health_cmd: str
    tokenizer_quality_gate: dict[str, Any]
    tokenizer_quality_gate_cmd: str
    has_train_runtime: bool
    compile_train_cmd: str
    compile_tokenizer_cmd: str
    profile_dashboard_cmd: str
    ...
    def to_json(self) -> dict[str, Any]:
        return {
            ...
            "probe_report_html_path": str(self.probe_report_html_path) if self.probe_report_html_path else None,
            "probe_report_html_uri": self.probe_report_html_path.resolve().as_uri() if self.probe_report_html_path else None,
            "probe_report_cmd": self.probe_report_cmd,
            ...
            "bridge_parity_cmd": self.bridge_parity_cmd,
            "regression_fast_cmd": self.regression_fast_cmd,
            ...
            "tokenizer_quality_gate": self.tokenizer_quality_gate,
            "tokenizer_quality_gate_cmd": self.tokenizer_quality_gate_cmd,
            "has_train_runtime": self.has_train_runtime,
            "compile_train_cmd": self.compile_train_cmd,
            "compile_tokenizer_cmd": self.compile_tokenizer_cmd,
            "profile_dashboard_cmd": self.profile_dashboard_cmd,
...
def _build_run_coverage(
    ...
    if kind == "train":
        sections = [
            ...
            _build_section(
                ...
                [
                    ...
                    ("gen_runtime", _has_run_artifact(run_dir, "generated_train_runtime_summary.json", "generated_train_runtime_summary_v7.json")),
                ],
                core=True,
            ),
            _build_section(
        ...
        if any(section["present"] < section["total"] for section in sections if section["core"]):
            next_actions.append({"label": "Refresh core training artifacts", "cmd": _build_run_make_cmd("v8-capture-artifacts-run", run_dir)})
        if any(section["present"] < section["total"] for section in sections if section["key"] == "runtime_perf"):
            next_actions.append({"label": "Capture training runtime telemetry", "cmd": _build_run_make_cmd("v8-profile-dashboard-run", run_dir)})
        if not _has_train_runtime(run_dir):
            next_actions.append({"label": "Compile train runtime (enables Training tab)", "cmd": _build_compile_train_cmd(run_dir)})
        ...
        bridge_enabled = isinstance(bridge_summary, dict) and bool(bridge_summary.get("bridge_mode"))
        if bridge_enabled:
            sections.append(
                _build_section(
                    "multimodal_bridge",
                    ...
                    [
                        ...
                        ("decoder_layout_prefill", _has_run_artifact(run_dir, "multimodal_bridge/decoder/layout_prefill.json")),
                    ],
                    core=True,
                )
            )
        ...
        bridge_section = next((section for section in sections if section["key"] == "multimodal_bridge"), None)
        if compile_section["present"] < compile_section["total"]:
            next_actions.append({"label": "Refresh v8 runtime artifacts", "cmd": _build_run_cmd(run_dir, bridge_summary)})
        if bridge_section is not None and bridge_section["present"] < bridge_section["total"]:
            next_actions.append({"label": "Rebuild multimodal bridge artifacts", "cmd": _build_run_cmd(run_dir, bridge_summary)})
        if basic_section["present"] < basic_section["total"]:
            next_actions.append({"label": "Promote v8 fast regression", "cmd": _build_regression_fast_cmd()})
...
def discover_run_dirs(models_root: Path) -> list[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(models_root):
        ...
        if "multimodal_bridge" in dirnames:
            bridge_report = current / "multimodal_bridge" / "bridge_report.json"
            if bridge_report.exists():
                runs.add(current)
        dirnames[:] = [d for d in dirnames if d not in SKIP_DIR_NAMES and d != "multimodal_bridge"]
...
def collect_run_record(run_dir: Path, models_root: Path) -> RunRecord:
    ...
    probe_report_html = _find_probe_report_html_path(run_dir)
    probe_report_cmd = _build_probe_report_cmd(run_dir)
    has_autopsy = _has_probe_autopsy(run_dir)
    has_probe_report_json = bool(sorted(run_dir.glob("*probe_report.json")))
    dsl_planner_cmd = _build_dsl_planner_cmd(run_dir) if (has_autopsy or has_probe_report_json) else None
    ...
    if probe_report_cmd:
        next_actions.append({"label": "Generate DSL probe report", "cmd": probe_report_cmd})
    bridge_parity_cmd = _build_bridge_parity_cmd(bridge_report_path)
    if bridge_parity_cmd:
        next_actions.append({"label": "Replay decoder first-token parity from bridge report", "cmd": bridge_parity_cmd})
    ...
    return RunRecord(
        ...
        probe_report_html_path=probe_report_html,
        probe_report_cmd=probe_report_cmd,
        ...
        run_cmd=_build_run_cmd(run_dir, bridge_summary),
        bridge_parity_cmd=bridge_parity_cmd,
        regression_fast_cmd=_build_regression_fast_cmd(),
        ...
        tokenizer_quality_gate=_extract_tokenizer_quality_gate(run_dir),
        tokenizer_quality_gate_cmd=_build_tokenizer_quality_gate_cmd(run_dir),
        has_train_runtime=_has_train_runtime(run_dir),
        compile_train_cmd=_build_compile_train_cmd(run_dir),
        compile_tokenizer_cmd=_build_compile_tokenizer_cmd(),
        profile_dashboard_cmd=_build_profile_dashboard_cmd(run_dir),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .badge.warn { color: #e0b860; background: rgba(255, 176, 32, 0.10); border-color: rgba(255, 176, 32, 0.18); }
...
<body>
      ...
      <div class="hero-grid">
        <div>
          <div class="brand-badge">
            ...
            <span class="brand-label">C-Kernel-Engine <span>v8</span></span>
          </div>
          <p class="eyebrow">Research Run Hub</p>
          <h1 class="headline">Run Hub<span class="accent">Operations Deck</span></h1>
          <p class="lede">Scan every v8 runtime root, spot weak bridge and IR artifacts fast, and jump straight into the right report or replay command.</p>
          <div class="hero-tags">
  ...
  <script>
    ...
    function cmdBlock(cmd, label, desc) {
      if (!cmd) return '';
      return `<div style="margin-top:8px;min-width:0;">
        ...
        <div class="spotlight-command" style="display:flex;align-items:center;gap:8px;min-width:0;">
          <code style="flex:1;min-width:0;overflow-x:auto;word-break:break-all">${escapeHtml(cmd)}</code>
    ...
    function renderCommandsPanel(run) {
      return `
        <div style="margin-top:14px;border:1px solid rgba(255,180,0,0.14);border-radius:12px;padding:14px 16px;background:rgba(20,20,20,0.6);min-width:0;overflow:hidden;">
          ...
          ${cmdBlock(run.generate_report_cmd, '📊 Generate v8 IR Report', 'Rebuilds ir_report.html from the current v8 artifacts, including multimodal bridge metadata when present.')}
          ${cmdBlock(run.runCmd, run.bridgeReportReady ? '🖼️ Replay VL Run' : '▶️ Replay Runtime', run.bridgeReportReady ? 'Replays the multimodal model/image/prompt lane inferred from bridge_report.json.' : 'Runs the cached v8 model root again through cks-v8-run.')}
          ${cmdBlock(run.bridgeParityCmd, '🧪 Bridge First-Token Parity', 'Replays the segmented multimodal prompt from bridge_report.json and checks decoder first-token alignment.')}
          ${cmdBlock(run.regressionFastCmd, '🚦 Promoted Fast Regression', 'Runs the promoted v8 end-to-end smoke lane, including Qwen3-VL coverage when configured.')}
        </div>
      `;
    }
    ...
    const runs = Array.isArray(HUB.runs) ? HUB.runs.map((run) => {
      ...
      const normalized = {
        ...
        probeReportHtmlReady: Boolean(run.probe_report_html_path),
        probeReportCmd: run.probe_report_cmd || '',
        tokenizerGate: (run.tokenizer_quality_gate && typeof run.tokenizer_quality_gate === 'object') ? run.tokenizer_quality_gate : {},
        tokenizerGateCmd: run.tokenizer_quality_gate_cmd || '',
        hasTrainRuntime: Boolean(run.has_train_runtime),
        compileTrainCmd: run.compile_train_cmd || '',
        compileTokenizerCmd: run.compile_tokenizer_cmd || '',
        profileDashboardCmd: run.profile_dashboard_cmd || '',
        runCmd: run.run_cmd || '',
        bridgeParityCmd: run.bridge_parity_cmd || '',
    ...
    const state = {
      ...
      svg: 'all',
      sort: 'updated_desc',
      view: loadPreference('ck_v8_run_hub_view', 'cards'),
      density: loadPreference('ck_v8_run_hub_density', 'comfortable'),
      selected: new Set(),
    };
        ...
        return `
          <article class="run-card">
            ...
            <div class="actions" style="margin-top:14px;">
              ...
              ${run.probe_report_html_uri ? `<a class="btn dsl-planner" target="_blank" rel="noopener" href="${escapeHtml(run.probe_report_html_uri)}">🔬 DSL Probe</a>` : (run.probe_report_cmd ? `<button class="btn dsl-planner-gen" data-copy="${encodeURIComponent(run.probe_report_cmd)}">⚡ Gen DSL Probe</button>` : '')}
  ...
  <div class="page">
    ...
    function renderHero() {
      ...
      const metrics = [
        ...
        { label: 'VL / Bridge Runs', value: fmtInt(bridgeReportCount), note: 'Runs with multimodal_bridge/bridge_report.json.' },
          ...
          <div class="spotlight-shell">
            <div class="spotlight-main">
              ...
              <div class="spotlight-summary">
                ...
                <div class="summary-tile"><div class="k">Lane</div><div class="v">${escapeHtml(run.bridgeMode === 'encoder_decoder' ? 'encoder + decoder' : 'decoder only')}</div></div>
                <div class="summary-tile"><div class="k">Prefix Tokens</div><div class="v">${escapeHtml(fmtInt(run.bridgeSummary.prefix_tokens))}</div></div>
              ...
              <div class="action-row">
                ${run.report_uri ? `<a class="btn primary" target="_blank" rel="noopener" href="${escapeHtml(run.report_uri)}">Open report</a>` : ''}
                ${run.bridge_report_uri ? `<a class="btn" target="_blank" rel="noopener" href="${escapeHtml(run.bridge_report_uri)}">Bridge report</a>` : ''}
                ${run.artifact_index_uri ? `<a class="btn" target="_blank" rel="noopener" href="${escapeHtml(run.artifact_index_uri)}">Artifacts</a>` : ''}
                ${run.dataset_viewer_uri ? `<a class="btn dataset" target="_blank" rel="noopener" href="${escapeHtml(run.dataset_viewer_uri)}">Dataset viewer</a>` : ''}
                ...
                ${run.probe_report_html_uri ? `<a class="btn dsl-planner" target="_blank" rel="noopener" href="${escapeHtml(run.probe_report_html_uri)}">🔬 DSL Probe Viewer</a>` : (run.probe_report_cmd ? `<button class="btn dsl-planner-gen" data-copy="${encodeURIComponent(run.probe_report_cmd)}">⚡ Gen DSL Probe</button>` : '')}
                ${embViewerLink(run, '🧬 Embeddings')}
                ${attnViewerLink(run, '🔭 Attention')}
              ...
              <details class="detail-toggle">
                ...
                <div class="spotlight-summary" style="margin-top:12px;">
                  ...
                  <div class="summary-tile"><div class="k">Image Source</div><div class="v">${escapeHtml(run.bridgeSummary.image_source || 'n/a')}</div></div>
                  <div class="summary-tile"><div class="k">Image Size</div><div class="v">${Array.isArray(run.bridgeSummary.source_image_size) ? escapeHtml(run.bridgeSummary.source_image_size.join(' × ')) : 'n/a'}</div></div>
                ...
                ${run.bridgeSummary.prompt_excerpt ? `<div class="health-note" style="margin-top:10px;">Prompt: ${escapeHtml(run.bridgeSummary.prompt_excerpt)}</div>` : ''}
                ${run.bridgeSummary.generated_excerpt ? `<div class="health-note" style="margin-top:6px;">Output: ${escapeHtml(run.bridgeSummary.generated_excerpt)}</div>` : ''}
            ...
            <aside class="spotlight-side">
              ...
              <div class="mini-grid">
                ...
                <div class="mini-tile"><div class="k">VL Lane</div><div class="v">${run.bridgeReportReady ? (run.bridgeMode === 'encoder_decoder' ? 'vision bridge ready' : 'bridge report ready') : 'decoder only'}</div></div>
    ...
    function renderActions() {
      els.actionPanel.innerHTML = `
        ...
        <div class="rail-body">
          <div class="kv-stack">
            <div class="kv-row" style="border-bottom:1px solid rgba(255,180,0,0.15);padding-bottom:8px;margin-bottom:4px"><span class="k" style="color:var(--gold)">⚙️ Build Prerequisites</span><span class="v mono" style="font-size:0.72rem">Run once after checkout or toolchain update</span></div>
            <div class="kv-row"><span class="k">C Tokenizer Runtime</span><span class="v mono">make tokenizer</span></div>
            <div class="kv-row"><span class="k">Full v8 Build</span><span class="v mono">make v8</span></div>
            <div class="kv-row" style="border-bottom:1px solid rgba(255,255,255,0.06);padding-bottom:8px;margin-bottom:4px"><span class="k">BPE Trainer (optional)</span><span class="v mono">make ck-bpe-train</span></div>
            <div class="kv-row"><span class="k">Hub</span><span class="v mono">python3 version/v8/tools/open_ir_hub_v8.py --open</span></div>
            <div class="kv-row"><span class="k">Refresh Report</span><span class="v mono">python3 version/v8/tools/open_ir_visualizer_v8.py --generate --run /path/to/run --html-only --strict-run-artifacts</span></div>
            <div class="kv-row"><span class="k">Replay Runtime</span><span class="v mono">version/v8/scripts/cks-v8-run run /path/to/run</span></div>
            <div class="kv-row"><span class="k">Bridge Parity</span><span class="v mono">python3 version/v8/scripts/decoder_first_token_parity_v8.py --bridge-report /path/to/run/multimodal_bridge/bridge_report.json</span></div>
            <div class="kv-row"><span class="k">Promoted Regression</span><span class="v mono">make regression-fast</span></div>
          </div>
          <div class="codebox" style="margin-top:14px;">Use the hub to choose a run. Use the replay and report commands to refresh decoder-only and encoder+decoder bridge artifacts without reconstructing the multimodal command chain by hand.</div>
        </div>
      `;
    }
          ...
          <article class="run-card">
            <div class="run-card-body">
              <div class="run-header">
                ...
                <div class="run-badges">
                  ...
                  ${(() => {
                    ...
                    if (tg.verdict === 'WARN') return '<span class="badge warn" title="Tokenizer has quality warnings">tok ⚠️</span>';
                    if (tg.verdict === 'PASS') return '<span class="badge pass" title="Tokenizer quality gate passed">tok ✓</span>';
                    return '';
                  })()}
                  ${run.kind === 'train' ? (run.hasTrainRuntime ? '<span class="badge pass" title="Training tab available: libtrain.so + summary present">train ✓</span>' : '<span class="badge warn" title="Training tab disabled: run compile_train_cmd to generate libtrain.so">train ⚠️</span>') : ''}
                </div>
              </div>
              ...
              <div class="run-stats">
                ...
                <div class="run-stat"><div class="k">Lane</div><div class="v">${escapeHtml(run.bridgeMode === 'encoder_decoder' ? 'encoder+decoder' : 'decoder')}</div></div>
                ${run.bridgeReportReady ? `<div class="run-stat"><div class="k">Prefix</div><div class="v">${escapeHtml(fmtInt(run.bridgeSummary.prefix_tokens))}</div></div>` : ''}
                <div class="run-stat"><div class="k">Final Loss</div><div class="v">${escapeHtml(fmtLoss(run.final_loss))}</div></div>
                ${run.probeSummary && run.probeSummary.probe_count ? `
                  <div class="run-stat"><div class="k">Probe Exact</div><div class="v">${escapeHtml(fmtPct(run.probeSummary.metrics ? run.probeSummary.metrics.exact_rate : null))}</div></div>
                  <div class="run-stat"><div class="k">Probe Count</div><div class="v">${escapeHtml(fmtInt(run.probeSummary.probe_count))}</div></div>
                ` : ''}
              </div>

              <div class="run-actions">
                ${run.report_uri ? `<a class="btn primary" target="_blank" rel="noopener" href="${escapeHtml(run.report_uri)}">Report</a>` : ''}
                ${run.bridge_report_uri ? `<a class="btn" target="_blank" rel="noopener" href="${escapeHtml(run.bridge_report_uri)}">Bridge</a>` : ''}
                ${run.artifact_index_uri ? `<a class="btn" target="_blank" rel="noopener" href="${escapeHtml(run.artifact_index_uri)}">Artifacts</a>` : ''}
                ${run.dataset_viewer_uri ? `<a class="btn dataset" target="_blank" rel="noopener" href="${escapeHtml(run.dataset_viewer_uri)}">Dataset</a>` : ''}
                ${run.probe_report_html_uri ? `<a class="btn dsl-planner" target="_blank" rel="noopener" href="${escapeHtml(run.probe_report_html_uri)}">🔬 DSL Probe</a>` : (run.probe_report_cmd ? `<button class="btn dsl-planner-gen" data-copy="${encodeURIComponent(run.probe_report_cmd)}">⚡ Gen Probe</button>` : '')}
              ...
              <details class="detail-toggle">
                ...
                ` : ''}
                ${run.bridgeSummary.prompt_excerpt ? `<div class="health-note" style="margin-bottom:10px;">Prompt: ${escapeHtml(run.bridgeSummary.prompt_excerpt)}</div>` : ''}
                ...
                <div class="health-note" style="margin-top:10px;">Advanced checks: ${escapeHtml(run.advancedReason || 'none tracked')}</div>
                <div class="run-stats">
                  <div class="run-stat"><div class="k">Image</div><div class="v">${Array.isArray(run.bridgeSummary.source_image_size) ? escapeHtml(run.bridgeSummary.source_image_size.join(' × ')) : 'n/a'}</div></div>
    ...
    function bind() {
      ...
      onEl('viewSelect', 'change', (event) => {
        ...
        savePreference('ck_v8_run_hub_view', state.view);
        refresh();
      });
      onEl('densitySelect', 'change', (event) => {
        state.density = event.target.value || 'comfortable';
        savePreference('ck_v8_run_hub_density', state.density);
        refresh();
      });
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate a parent IR Run Hub page for CK v8 runtime roots.")
    p.add_argument(
        "--models-root",
        type=Path,
        default=Path.home() / ".cache" / "ck-engine-v8" / "models",
        help="Root directory that contains run/model folders (default: ~/.cache/ck-engine-v8/models)",
    )
    p.add_argument(
        "--output",
        type=Path,
