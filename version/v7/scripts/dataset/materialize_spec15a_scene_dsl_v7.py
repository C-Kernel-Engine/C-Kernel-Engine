#!/usr/bin/env python3
"""Materialize a cache-local spec15a memory-map workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec15a_v7.py"
...
def _row_repeat(prompt_surface: str, *, canonical_repeat: int, bridge_repeat: int) -> int:
    ...
    if prompt_surface == "bridge_count_guard":
        return max(1, bridge_repeat + 1)
    if prompt_surface in {"repair_strip_prompt_tags", "repair_singletons"}:
        return max(1, bridge_repeat - 2)
    if prompt_surface in {"repair_style_bundle", "repair_no_prompt_echo"}:
        return max(1, bridge_repeat - 1)
    if prompt_surface == "train_hidden_style_bundle":
        return max(1, bridge_repeat + 2)
    if prompt_surface == "train_hidden_component_binding":
        return max(1, bridge_repeat + 1)
...
def materialize_workspace(
    ...
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"spec15a generator failed (rc={proc.returncode}):\n{proc.stdout.strip()}")
    ...
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec15a_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec15a probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec15a is the strict domain-agnostic memory_map line: compact scene DSL only, payload externalized, and one bounded visual family."
            }
        }
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec15a memory-map workspace")
    ...
    ap.add_argument("--prefix", default="spec15a_scene_dsl", help="Dataset prefix")
    ap.add_argument("--pretrain-canonical-repeat", type=int, default=20)
    ap.add_argument("--pretrain-bridge-repeat", type=int, default=6)
    ap.add_argument("--midtrain-canonical-repeat", type=int, default=16)
    ap.add_argument("--midtrain-bridge-repeat", type=int, default=6)
