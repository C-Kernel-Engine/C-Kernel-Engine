# ============================================================================
# ORIGIN: Copied from v4/build_ir_v4.py for v7 independence
# PURPOSE: HF model conversion and weight inspection utilities
# NOTE: Compatibility layer - consider migrating functionality to v7 native
# ============================================================================

#!/usr/bin/env python3
"""
build_ir_v7.py - IR v7 pipeline
...
# Import from ir_types_v7 for v7 types
# Use absolute import to avoid relative import issues
_ir_types_path = Path(__file__).parent / "ir_types_v7.py"
if _ir_types_path.exists():
    import importlib.util
    spec = importlib.util.spec_from_file_location("ir_types_v7", _ir_types_path)
    ir_v7 = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ir_v7)
    # Re-export types for code that uses v3.XXX
    DTYPE_BYTES = ir_v7.DTYPE_BYTES
    QUANT_BLOCK = ir_v7.QUANT_BLOCK
    Buffer = ir_v7.Buffer
    Weight = ir_v7.Weight
    Op = ir_v7.Op
    LayerIR = ir_v7.LayerIR
    GraphIR = ir_v7.GraphIR
    LoweredIR = ir_v7.LoweredIR
    Layout = ir_v7.Layout
    ModelLayout = ir_v7.ModelLayout  # Alias
    Schedule = ir_v7.Schedule
    OpType = ir_v7.OpType
    DType = ir_v7.DType
    Mode = ir_v7.Mode
    # Re-export helper functions
    make_buffer_name = ir_v7.make_buffer_name
    compute_buffer_size = ir_v7.compute_buffer_size
    load_registry_ops = ir_v7.load_registry_ops
    build_op_type_enum = ir_v7.build_op_type_enum
    normalize_kernel_registry = ir_v7.normalize_kernel_registry
    resolve_kernel_template = ir_v7.resolve_kernel_template
    parse_config = ir_v7.parse_config
    model_id_to_name = ir_v7.model_id_to_name
    # Re-export constants
    DEFAULT_KERNEL_REGISTRY_PATH = ir_v7.DEFAULT_KERNEL_REGISTRY_PATH
else:
    # Fallback if ir_types_v7 not available yet
    DTYPE_BYTES = {
...
# Aliases for v3 compatibility (these come from ir_types_v7)
CACHE_LINE = 64
...
import codegen_v7 as codegen_v7_impl  # Alias for backward compatibility
...
try:
    ...
    # v7 scripts are often executed directly, where only this directory is on sys.path.
    # Fall back to the shared top-level scripts/training_config.py if present.
    _ROOT_SCRIPTS = Path(__file__).resolve().parents[3] / "scripts"
    if _ROOT_SCRIPTS.is_dir():
        _root_scripts_str = str(_ROOT_SCRIPTS)
        if _root_scripts_str not in sys.path:
            sys.path.insert(0, _root_scripts_str)
    try:
        import training_config as tc
    except ModuleNotFoundError as _tc_err:
        class _MissingTrainingConfig:
            def __getattr__(self, name):
                raise ModuleNotFoundError(
                    "training_config module not found for ir_core_v7. "
                    "Expected version/v7/scripts/training_config.py or scripts/training_config.py"
                ) from _tc_err

        tc = _MissingTrainingConfig()
...
def emit_c_header(layout: ModelLayout, path: str, extra_api: str = "") -> None:
    """Emit comprehensive C header with all layout constants, structures, and macros.

    Generates everything that codegen_v7.py expects:
    - Model constants (TOTAL_BYTES, NUM_LAYERS, EMBED_DIM, etc.)
    - Layer offset structure and array
    - Header/Footer/Globals structures
    - PTR macro for pointer arithmetic
    - Canary definitions
    - Extra API (model struct, function declarations)
    """
    # Create safe model name for C identifiers
    ...
    head_dim = config.get("head_dim", embed_dim // num_heads if num_heads else 64)
    vocab_size = config.get("vocab_size", 0)
    max_seq_len = config.get("max_seq_len", config.get("max_position_embeddings", 32768))
    intermediate_dim = config.get("intermediate_dim", config.get("intermediate_size", 0))
    rms_eps = config.get("rms_eps", config.get("rms_norm_eps", 1e-6))

    # Aligned dimensions (default to raw if not specified)
    aligned_embed = config.get("aligned_embed", embed_dim)
    aligned_head = config.get("aligned_head", head_dim)
    aligned_intermediate = config.get("aligned_intermediate", intermediate_dim)
    aligned_context = config.get("aligned_context", max_seq_len)

    # Get dtype bytes (assume fp32 for now)
    dtype_bytes = 4

    lines = []
    add = lines.append

    # Header guard using model name
    guard_name = f"{safe_name}_LAYOUT_H"
    add(f"#ifndef {guard_name}")
    add(f"#define {guard_name}")
    add("")
    add("/* AUTO-GENERATED - DO NOT EDIT */")
    add(f"/* Model: {layout.name} */")
    add("")
    ...
    add(" * ============================================================================ */")
    add("")
    add(f"#define {safe_name}_MAGIC 0x{0x42554D50:08X}  /* 'BUMP' */")
    add(f"#define {safe_name}_VERSION 6")
    ...
    add(f"#define {safe_name}_DTYPE_BYTES {dtype_bytes}")
    add("")
    add(f"#define {safe_name}_NUM_LAYERS {num_layers}")
    add(f"#define {safe_name}_EMBED_DIM {embed_dim}")
    add(f"#define {safe_name}_NUM_HEADS {num_heads}")
    add(f"#define {safe_name}_NUM_KV_HEADS {num_kv_heads}")
    add(f"#define {safe_name}_HEAD_DIM {head_dim}")
    add(f"#define {safe_name}_VOCAB_SIZE {vocab_size}")
    add(f"#define {safe_name}_MAX_SEQ_LEN {max_seq_len}")
    add(f"#define {safe_name}_INTERMEDIATE_DIM {intermediate_dim}")
    add(f"#define {safe_name}_RMS_EPS {rms_eps}f")
    add("")
    add(f"#define {safe_name}_ALIGNED_EMBED {aligned_embed}")
    add(f"#define {safe_name}_ALIGNED_HEAD {aligned_head}")
    add(f"#define {safe_name}_ALIGNED_INTERMEDIATE {aligned_intermediate}")
    add(f"#define {safe_name}_ALIGNED_CONTEXT {aligned_context}")
    add("")

    # ==== Canary Definitions ====
    add("/* ============================================================================")
    add(" * CANARY DEFINITIONS")
    add(" * ============================================================================ */")
    add("")
    add(f"#define {safe_name}_CANARY_VALUE 0xDEADBEEFU")
    add(f"#define {safe_name}_CANARY_SIZE 16")
    ...
    if layout.canaries:
        ...
        for canary in layout.canaries:
            if hasattr(canary, 'name') and hasattr(canary, 'offset'):
                add(f'    {{"{canary.name}", {canary.offset}ULL}},')
            elif isinstance(canary, dict):
                add(f'    {{"{canary.get("name", "unknown")}", {canary.get("offset", 0)}ULL}},')
        add("};")
    else:
        add(f"static const {safe_name}Canary {safe_name}_CANARIES[1] = {{{{\"none\", 0}}}};")
    add("")
    ...
    add(f"    ((void*)((uint8_t*)((model)->weights_base) + (offset)))")
    add("")
            ...
            for buf in first_layer.buffers:
                ...
                if hasattr(buf, 'name'):
                    ...
                    if len(parts) == 3 and parts[0] == "layer":
                        layer_buffer_names.append(parts[2])

    add("/* ============================================================================")
    add(" * LAYER OFFSETS STRUCTURE")
    add(" * ============================================================================ */")
    add("")
    add(f"typedef struct {{")
    for field in sorted(set(layer_buffer_names)):
        safe_field = field.replace("-", "_")
        add(f"    uint64_t {safe_field};")
    add(f"}} {safe_name}LayerOffsets;")
    add("")

    # ==== Layer Offsets Array ====
    if section and hasattr(section, 'layers') and section.layers:
        add(f"static const {safe_name}LayerOffsets {safe_name}_LAYERS[{num_layers}] = {{")
                ...
                for buf in layer.buffers:
                    if hasattr(buf, 'name'):
                        ...
                        if len(parts) == 3 and parts[0] == "layer":
                            ...
                            offset = buf.offset if hasattr(buf, 'offset') else 0
                            layer_offsets[field] = offset
            for field in sorted(set(layer_buffer_names)):
                safe_field = field.replace("-", "_")
                add(f"        .{safe_field} = {layer_offsets.get(safe_field, 0)}ULL,")
            add("    },")
        add("};")
    else:
        add(f"static const {safe_name}LayerOffsets {safe_name}_LAYERS[1] = {{{{{0}}}}};")
    add("")

    # ==== Header Buffers Structure ====
    header_fields = []
    header_offsets = {}
    if section and hasattr(section, 'header_buffers'):
        for buf in section.header_buffers:
            if hasattr(buf, 'name'):
                field = buf.name.replace(".", "_").replace("-", "_")
                header_fields.append(field)
                header_offsets[field] = buf.offset if hasattr(buf, 'offset') else 0

    add("/* ============================================================================")
    add(" * HEADER STRUCTURE (embedding, vocab)")
    add(" * ============================================================================ */")
    add("")
    add(f"typedef struct {{")
    for field in header_fields:
        add(f"    uint64_t {field};")
    add(f"}} {safe_name}HeaderOffsets;")
    add("")
    add(f"static const {safe_name}HeaderOffsets {safe_name}_HEADER = {{")
    for field in header_fields:
        add(f"    .{field} = {header_offsets.get(field, 0)}ULL,")
    add("};")
    add("")

    # ==== Footer Buffers Structure ====
    footer_fields = []
    footer_offsets = {}
    if section and hasattr(section, 'footer_buffers'):
        for buf in section.footer_buffers:
            if hasattr(buf, 'name'):
                field = buf.name.replace(".", "_").replace("-", "_")
                footer_fields.append(field)
                footer_offsets[field] = buf.offset if hasattr(buf, 'offset') else 0

    add("/* ============================================================================")
    add(" * FOOTER STRUCTURE (final layernorm, lm_head, logits)")
    add(" * ============================================================================ */")
    add("")
    add(f"typedef struct {{")
    ...
    for field in footer_fields:
        add(f"    .{field} = {footer_offsets.get(field, 0)}ULL,")
    add("};")
    add("")

    # ==== Globals Structure (RoPE caches) ====
    globals_fields = []
    globals_offsets = {}
    if section and hasattr(section, 'globals'):
        for buf in section.globals:
            if hasattr(buf, 'name'):
                field = buf.name.replace(".", "_").replace("-", "_")
                globals_fields.append(field)
                globals_offsets[field] = buf.offset if hasattr(buf, 'offset') else 0

    add("/* ============================================================================")
    add(" * GLOBALS STRUCTURE (RoPE caches)")
    add(" * ============================================================================ */")
    add("")
    add(f"typedef struct {{")
    for field in globals_fields:
        add(f"    uint64_t {field};")
    if not globals_fields:
        add("    uint64_t _placeholder;  /* empty struct placeholder */")
    add(f"}} {safe_name}GlobalsOffsets;")
    add("")
    add(f"static const {safe_name}GlobalsOffsets {safe_name}_GLOBALS = {{")
    for field in globals_fields:
        add(f"    .{field} = {globals_offsets.get(field, 0)}ULL,")
    if not globals_fields:
        add("    ._placeholder = 0,")
    add("};")
    add("")
    ...
    if extra_api:
        add("/* ============================================================================")
        add(" * MODEL STRUCT AND API")
        add(" * ============================================================================ */")
        add("")
        if isinstance(extra_api, list):
            for line in extra_api:
                add(line)
        else:
            add(extra_api)
        add("")
...
def parse_args(argv: List[str]) -> Dict:
    ...
    args = {
        ...
        "codegen": "v4",  # Codegen version: v4 (legacy) or v7 (explicit unrolled)
        # Training options
    ...
    for arg in argv:
        ...
        if arg.startswith("--config="):
            ...
            if codegen_val not in ("v4", "v7"):
                raise ValueError(f"--codegen must be v4/v7, got: {codegen_val}")
...
def print_usage():
    print("Usage:")
    print("  python version/v7/scripts/build_ir_v7.py MODEL [OPTIONS]")
    print("  python version/v7/scripts/build_ir_v7.py --config=FILE [OPTIONS]")
    print("  python version/v7/scripts/build_ir_v7.py --preset=NAME [OPTIONS]")
    print()
    print("Options:")
    ...
    print("  --codegen=v4|v7         Codegen version (default: v4)")
    print("                          v4: Loop-based with runtime dtype dispatch")
    print("                          v7: Explicit unrolled (each layer separate, explicit kernels)")
        ...
        def emit_mode_outputs(enable_fusion: bool) -> None:
            ...
            if mode in ("prefill", "decode"):
                if config["dtype"] != "fp32":
                    print("[WARN] v4/v7 codegen currently emits fp32 activations only. Use --dtype=fp32 for runnable C.")
                codegen_version = args.get("codegen", "v4")
                if codegen_version == "v7":
                    print(f"[CODEGEN] Using v7 (explicit unrolled) for {mode}")
                    generated_c = codegen_v7_impl.generate(
                        Path(lowered_path),
                        Path(layout_json_path),
                        debug=args.get("debug", False),
                        init_call=None,
                        profile=False,
                        dump=args.get("parity", False),
                    )
                    with open(os.path.join(output_dir, f"generated_{safe_name}.c"), "w") as f:
                        f.write(generated_c)
                else:
