...
$(LIB_AUDIO): $(LIB) $(BUILD_STAMP) src/kernels/audio_kernels.c src/kernels/audio_lstm_scan.c src/ckernel_strict.c src/ck_threadpool.c include/ckernel_audio.h
	$(CC) $(CFLAGS) -shared -o $@ src/kernels/audio_kernels.c src/kernels/audio_lstm_scan.c src/ckernel_strict.c src/ck_threadpool.c -L$(BUILD_DIR) -lckernel_engine -Wl,-rpath,'$$ORIGIN' -lm -lpthread
...
test-audio-v8-contracts:
	...
	$(PYTHON) $(PYTHONFLAGS) -m pytest -q tests/test_v8_cohere_generated_encoder.py
	$(PYTHON) $(PYTHONFLAGS) -m pytest -q tests/test_v8_cohere_generated_decoder.py
