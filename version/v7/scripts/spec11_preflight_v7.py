#!/usr/bin/env python3
"""Preflight gates for spec11 keyed scene-DSL training."""
...
from build_spec11_probe_contract_v7 import build_contract as build_probe_contract
...
from render_svg_structured_scene_spec11_v7 import _parse_scene_document, render_structured_scene_spec11_svg
...
def _validate_scene_output(
    ...
    try:
        rendered_svg = render_structured_scene_spec11_svg(output_tokens, content=content_json)
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec11 render catalog must be a JSON list")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Compute spec11 training budgets and run keyed scene-DSL preflight gates")
    ap.add_argument("--run", required=True, type=Path, help="Run directory containing the staged dataset")
    ap.add_argument("--prefix", default="spec11_keyed_scene_dsl", help="Dataset prefix")
    ...
    print("[spec11-preflight] " f"catalog pass={report['catalog']['pass']} rows={report['catalog']['count']} pass_rate={report['catalog']['pass_rate']:.4f}")
    print("[spec11-preflight] " f"canary pass={report['canary']['pass']} cases={report['canary']['count']} pass_rate={report['canary']['pass_rate']:.4f}")

    if args.strict and not passed:
        raise SystemExit("spec11 preflight failed: " + ", ".join(reasons))
    return 0
