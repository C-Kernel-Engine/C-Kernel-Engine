#!/usr/bin/env python3
"""
Validate v6.6 tooling contracts across IR/codegen/parity orchestration.
...
def run_checks() -> list[CheckRow]:
    ...
    codegen_py = SCRIPTS / "codegen_v6_6.py"
    ...
    template_names = _extract_template_names(V66 / "templates")
    ...
    rows.append(
        CheckRow(
            ...
            status=l5_status,
            detail=(
                f"token defaults externalized via probe_defaults.py; users can set CK_V66_PROBE_* env vars; users={','.join(sorted(defaults_module_used))}"
    ...
    has_target = "v6.6-validate-contracts:" in makefile
    has_preflight_hook = "v6.6-validate-contracts" in makefile and (
        "ci-local-fast" in makefile or "e2e-v66" in makefile
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate v6.6 tooling contracts")
    ...
    rows = run_checks()
    print("=" * 112)
    print("v6.6 TOOLING CONTRACT REPORT")
    print("=" * 112)
