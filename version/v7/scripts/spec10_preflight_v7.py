#!/usr/bin/env python3
"""Preflight gates for spec10 asset-grounded scene-DSL training."""
...
from build_spec10_probe_contract_v7 import build_contract as build_probe_contract
...
def _validate_scene_output(output_tokens: str, expected_svg: str | None, prompt: str) -> dict[str, Any]:
    ...
    try:
        rendered_svg = render_structured_scene_spec09_svg(output_tokens)
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec10 render catalog must be a JSON list")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Compute spec10 training budgets and run asset-grounded scene-DSL preflight gates")
    ap.add_argument("--run", required=True, type=Path, help="Run directory containing the staged dataset")
    ap.add_argument("--prefix", default="spec10_asset_scene_dsl", help="Dataset prefix")
    ...
    print("[spec10-preflight] " f"catalog pass={report['catalog']['pass']} rows={report['catalog']['count']} pass_rate={report['catalog']['pass_rate']:.4f}")
    print("[spec10-preflight] " f"canary pass={report['canary']['pass']} cases={report['canary']['count']} pass_rate={report['canary']['pass_rate']:.4f}")

    if args.strict and not passed:
        raise SystemExit("spec10 preflight failed: " + ", ".join(reasons))
    return 0
