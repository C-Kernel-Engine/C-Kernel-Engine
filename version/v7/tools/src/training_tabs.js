...
function buildDataLabReadinessSection(files) {
    ...
    if (regimenStages.length === 0) {
        return `
            <div class="parity-section" style="margin-top:0.8rem;">
                <h3><span class="badge badge-blue">Training Readiness</span> Parity Gate - Stage-by-Stage</h3>
                <div style="color:var(--text-muted);font-size:0.74rem;line-height:1.6;">
                    No parity regimen stages are loaded. Generate <code>training_parity_regimen_latest.json</code> to restore the A/B/C/D/E/F readiness cards.
                </div>
            </div>
        `;
    }
    ...
    const groupHtml = groupOrder.map((group) => {
        ...
        const groupColor = passCount === items.length ? '#6ee7b7' : (passCount === 0 ? '#fca5a5' : '#fbbf24');
        const cards = items.map(({ id, row, info }) => {
            const pass = String(row.status || '').toUpperCase() === 'PASS';
            const border = pass ? '#47b475' : '#f87171';
            const bg = pass ? 'rgba(71,180,117,0.07)' : 'rgba(248,113,113,0.07)';
        ...
        return `
            <div style="margin-bottom:0.9rem;">
                <div style="font-size:0.72rem;font-weight:700;color:${groupColor};margin-bottom:0.38rem;letter-spacing:0.03em;text-transform:uppercase;">
                    ${htmlEscape(group)} <span style="opacity:0.6;font-weight:400;text-transform:none;">(${passCount}/${items.length} pass)</span>
                </div>
