#!/usr/bin/env python3
"""Materialize a cache-local spec15b system_diagram workspace."""
...
STRUCTURED_GENERATOR = ROOT / "version" / "v7" / "scripts" / "generate_svg_structured_spec15b_v7.py"
LINE_NAME = "spec15b_scene_dsl"
...
def _row_repeat(prompt_surface: str, *, canonical_repeat: int, bridge_repeat: int) -> int:
    ...
    if prompt_surface in {"bridge_scene_only", "bridge_terminal_exact", "bridge_count_guard"}:
        return max(1, bridge_repeat + 1)
    ...
    if prompt_surface in {"repair_stage_uniqueness", "repair_terminal_panel"}:
        return max(1, bridge_repeat + 2)
    if prompt_surface == "repair_terminal_footer_tail":
        return max(1, bridge_repeat + 2)
    if prompt_surface in {"repair_stop_boundary", "repair_no_prompt_echo"}:
        return max(1, bridge_repeat + 2)
...
def materialize_workspace(
    ...
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"spec15b generator failed (rc={proc.returncode}):\n{proc.stdout.strip()}")
    ...
    probe_cmd = [
        python_exec,
        str(ROOT / "version" / "v7" / "scripts" / "build_spec15b_probe_contract_v7.py"),
        "--run",
        str(workspace),
        "--prefix",
        prefix,
    ...
    if probe_proc.returncode != 0:
        raise RuntimeError(f"spec15b probe contract build failed (rc={probe_proc.returncode}):\n{probe_proc.stdout.strip()}")
    ...
    workspace_manifest = {
        ...
        "stages": {
            ...
            "midtrain": {
                ...
                "notes": "Spec15b is the strict domain-agnostic system_diagram line: compact scene DSL only, payload externalized, and one bounded visual family."
            }
        }
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Materialize a cache-local spec15b system_diagram workspace")
    ...
    ap.add_argument("--prefix", default="spec15b_scene_dsl", help="Dataset prefix")
