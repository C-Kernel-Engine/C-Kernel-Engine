#!/usr/bin/env python3
"""Build a balanced minimal-pair probe contract for structured scene runs."""
...
def build_contract(run_dir: Path, prefix: str, per_split: int) -> dict[str, Any]:
    ...
    catalog_path = _find_existing(
        [
            ...
            run_dir / "spec04_workspace" / "manifests" / "generated" / "structured_atoms" / f"{prefix}_render_catalog.json",
            run_dir / "spec04_workspace" / "tokenizer" / f"{prefix}_render_catalog.json",
        ]
    )
    if catalog_path is None:
        raise SystemExit(f"render catalog not found for prefix '{prefix}' under dataset/ or spec04_workspace/")
    ...
    split_defs = [
        (
            "train",
            "Train",
            run_dir / "dataset" / "pretrain" / "train" / f"{prefix}_pretrain_train.txt",
        ),
        (
            "dev",
            "Dev",
            run_dir / "dataset" / "pretrain" / "dev" / f"{prefix}_pretrain_dev.txt",
        ),
        (
            "test",
            "Test",
            run_dir / "dataset" / "pretrain" / "test" / f"{prefix}_pretrain_test.txt",
        ),
    ]
    ...
    return {
        ...
        "title": f"{prefix} Structured Scenes Probe Report",
        "dataset_type": "svg",
        "output_adapter": {
            "name": "text_renderer",
            "stop_markers": ["[/svg]"],
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build a balanced minimal-pair probe contract for a structured scene run")
    ap.add_argument("--run", required=True, type=Path, help="Run directory containing structured scene dataset artifacts")
