#!/usr/bin/env python3
"""Build a balanced probe contract for spec16 generalized visual scene bundles."""
...
def _prompt_sort_key(prompt: str, catalog: dict[str, dict[str, Any]]) -> tuple[str, ...]:
    ...
    return (
        str(row.get("layout") or row.get("family") or ""),
...
def _prompt_case_bucket_key(prompt: str, catalog: dict[str, dict[str, Any]]) -> tuple[str, str, str]:
    ...
    stable_case = source_asset or case_id
    return (form, stable_case, case_id)
...
def _case_from_prompt(prompt: str, catalog: dict[str, dict[str, Any]], split: str, split_label: str, index: int) -> dict[str, Any]:
    row = catalog.get(prompt) or {}
    layout = str(row.get("layout") or row.get("family") or "")
...
def build_contract(
    ...
    *,
    ...
    return {
        ...
        "title": "Spec16 Generalized Visual Scene Bundle Probe",
        "dataset_type": "svg",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build a balanced probe contract for spec16 generalized visual scene bundles.")
