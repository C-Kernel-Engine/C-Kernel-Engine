...
def main() -> None:
    ...
    with open(args.gguf, "rb") as f:
        ...
        arch_prefixes = ("general.", "llama.", "qwen2.", "qwen.", "mistral3.", "mistral.")
        for _ in range(n_kv):
            key = r.key_str()
            vtype = r.u32()
        ...
        if wq0 and wk0 and wo0:
            ...
            if q_dim1 != embed_dim or k_dim1 != embed_kv:
                ...
                print(f"[info] Detected non-standard attention dimensions")
                print(f"  expected: Q={embed_dim}, K={embed_kv}, O={embed_dim}")
                print(f"  actual:   Q={q_dim1}, K={k_dim1}, O={o_dim0}")

                # Infer head dimensions
                inferred_q_head_dim = q_dim1 // num_heads if q_dim1 % num_heads == 0 else q_dim1
                inferred_kv_dim = k_dim1

                # Update for consistency with actual tensors
                embed_kv = k_dim1
                head_dim = inferred_q_head_dim

                print(f"  inferred: head_dim={head_dim}, embed_kv={embed_kv}")
        ...
        for layer in range(num_layers):
            ...
            if wq.ne0 != embed_dim:
                raise gguf.GGUFError(f"{wq.name}: ne0 mismatch: expected {embed_dim}, got {wq.ne0}")
            if wk.ne0 != embed_dim or wv.ne0 != embed_dim:
                raise gguf.GGUFError(f"K/V ne0 mismatch: expected {embed_dim}, got {wk.ne0}/{wv.ne0}")
            if wo.ne1 != embed_dim:
                raise gguf.GGUFError(f"{wo.name}: ne1 mismatch: expected {embed_dim}, got {wo.ne1}")

            # For ne1 dimensions, check against inferred values
            if wq.ne1 != embed_dim and wq.ne1 != head_dim * num_heads:
                raise gguf.GGUFError(f"{wq.name}: ne1 invalid: expected {embed_dim} or {head_dim * num_heads}, got {wq.ne1}")
            if wk.ne1 != embed_kv:
                raise gguf.GGUFError(f"{wk.name}: ne1 invalid: expected {embed_kv}, got {wk.ne1}")
            if wv.ne1 != embed_kv:
                raise gguf.GGUFError(f"{wv.name}: ne1 invalid: expected {embed_kv}, got {wv.ne1}")
            if wo.ne0 != wq.ne1:
                raise gguf.GGUFError(f"{wo.name}: ne0 mismatch with Q ne1: expected {wq.ne1}, got {wo.ne0}")
