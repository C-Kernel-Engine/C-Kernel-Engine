#!/usr/bin/env python3
"""Preflight gates for spec12 scene-DSL training."""
...
from build_spec12_probe_contract_v7 import build_contract as build_probe_contract
...
from spec12_scene_canonicalizer_v7 import parse_scene_document
...
def _budget_stage(
    *,
    ...
    bos_id, eos_id, pad_id = _resolve_special_ids(tokenizer_json=tokenizer_json, bos_override=None, eos_override=None, pad_override=None)
...
def _validate_scene_output(output_tokens: str, expected_svg: str | None, prompt: str, content_json: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        scene_doc = parse_scene_document(output_tokens)
        parse_ok = True
        parse_error = None
    ...
    try:
        rendered_svg = render_structured_scene_spec12_svg(output_tokens, content=content_json)
    ...
    return {
        ...
        "scene_doc": None if scene_doc is None else {"layout": scene_doc.layout, "topic": scene_doc.topic},
    }
...
def _catalog_validation(run_dir: Path, prefix: str) -> dict[str, Any]:
    ...
    if not isinstance(catalog_rows, list):
        raise SystemExit("spec12 render catalog must be a JSON list")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Preflight gates for spec12 scene-DSL training")
    ap.add_argument("--run", required=True, type=Path, help="Run directory")
    ap.add_argument("--prefix", default="spec12_scene_dsl", help="Dataset prefix")
